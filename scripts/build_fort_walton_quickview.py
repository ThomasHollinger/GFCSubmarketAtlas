...
OUT_PATH = ROOT / 'data' / 'market_quickview' / 'fort_walton_quickview_blocks.geojson'

MILES_PER_DEG_LAT = 69.172
CELL_MILES = 2.0
SUBMARKET_NAME = 'Fort Walton'
...
def main(zip_path: Path):
    ...
    if len(cells) != 39:
        raise RuntimeError(f'Expected 39 Fort Walton blocks from project boundary, generated {len(cells)}')
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        raise SystemExit('Usage: build_fort_walton_quickview.py /path/to/Fort Walton Zip.zip')
