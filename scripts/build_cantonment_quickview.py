...
OUT_PATH = ROOT / 'data' / 'market_quickview' / 'cantonment_quickview_blocks.geojson'

MILES_PER_DEG_LAT = 69.172
CELL_MILES = 2.0
SUBMARKET_NAME = 'Cantonment'
...
def main(zip_path: Path):
    ...
    if len(cells) != 48:
        raise RuntimeError(f'Expected 48 Cantonment blocks from project boundary, generated {len(cells)}')
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        raise SystemExit('Usage: build_cantonment_quickview.py /path/to/Cantonment.zip')
