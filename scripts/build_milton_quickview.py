...
OUT_PATH = ROOT / 'data' / 'market_quickview' / 'milton_quickview_blocks.geojson'

MILES_PER_DEG_LAT = 69.172
CELL_MILES = 2.0
SUBMARKET_NAME = 'Milton'
...
def main(zip_path: Path):
    ...
    if len(cells) != 42:
        raise RuntimeError(f'Expected 42 Milton blocks from project boundary, generated {len(cells)}')
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        raise SystemExit('Usage: build_pace_quickview.py /path/to/Pace.zip')
