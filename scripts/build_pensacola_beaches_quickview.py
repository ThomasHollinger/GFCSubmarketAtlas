...
OUT_PATH = ROOT / 'data' / 'market_quickview' / 'pensacola_beaches_quickview_blocks.geojson'

MILES_PER_DEG_LAT = 69.172
CELL_MILES = 2.0
SUBMARKET_NAME = 'Pensacola Beaches'
...
def main(zip_path: Path):
    ...
    if len(cells) != 45:
        raise RuntimeError(f'Expected 45 Pensacola Beaches blocks from project boundary, generated {len(cells)}')
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        raise SystemExit('Usage: build_pensacola_beaches_quickview.py /path/to/Pensacola Beaches.zip')
