...
OUT_PATH = ROOT / 'data' / 'market_quickview' / 'south_baldwin_quickview_blocks.geojson'

MILES_PER_DEG_LAT = 69.172
CELL_MILES = 2.0
SUBMARKET_NAME = 'South Baldwin'
...
def main(zip_path: Path):
    ...
    if len(cells) != 87:
        raise RuntimeError(f'Expected 87 South Baldwin blocks from project boundary, generated {len(cells)}')
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        raise SystemExit('Usage: build_south_baldwin_quickview.py /path/to/South-Baldwin.zip')
