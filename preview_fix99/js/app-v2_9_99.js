...
function showRadiusPrompt(mode, centerLatLng, radiusMiles) {
  ...
  if (iconEl) {
    // Prevent the button click from becoming a Leaflet map click.
    L.DomEvent.disableClickPropagation(iconEl);
    L.DomEvent.disableScrollPropagation(iconEl);
    const card = iconEl.querySelector('.radius-prompt-card');
    if (card) L.DomEvent.disableClickPropagation(card);
    const yes = iconEl.querySelector('[data-radius-prompt-action="yes"]');
    const no = iconEl.querySelector('[data-radius-prompt-action="no"]');
    if (yes) {
      L.DomEvent.on(yes, 'click', (event) => {
        L.DomEvent.stop(event);
        openRadiusPromptReport(mode);
      });
    }
    if (no) {
      L.DomEvent.on(no, 'click', (event) => {
        L.DomEvent.stop(event);
...
function openMarketSnapshotReportAt(centerLatLng, radiusMiles) {
  const radius = Number(radiusMiles);
  if (!centerLatLng || ![1, 3, 5, 10].includes(radius)) return false;
  const center = centerLatLng instanceof L.LatLng ? centerLatLng : L.latLng(centerLatLng.lat, centerLatLng.lng);
  state.marketSnapshot = state.marketSnapshot || { active: false, radiusMiles: null, awaitingPoint: false, busy: false, pendingCenter: null, radiusLayer: null, radiusCircle: null, promptMarker: null };
  state.marketSnapshot.pendingCenter = center;
  state.marketSnapshot.radiusMiles = radius;
  state.marketSnapshot.busy = false;

  // Remove only the temporary prompt/radius graphic. Do not reset the selected radius.
  clearRadiusPromptOverlay('snapshot');
  state.marketSnapshot.active = false;
  state.marketSnapshot.awaitingPoint = false;
  updateMarketSnapshotUI();
  ...
  // Render the modal shell first so the click always produces visible feedback even
  // if one of the optional data builders throws while data is still warming.
  openMarketSnapshotModal('Market Preview', subtitle, '<div class="snapshot-empty">Loading Market Preview…</div>');
  ...
  const refreshPreview = () => {
    ...
    try {
      ...
      body.innerHTML = `<div class="snapshot-empty">Market Preview could not be rendered: ${escapeHtml(err?.message || err)}</div>`;
    }
  };

  // Render once immediately, then refresh as the heavier sources finish loading.
  refreshPreview();
  Promise.resolve(ensureAcsMeanIncomeLoaded()).then(refreshPreview).catch(err => console.warn('Market Preview ACS refresh failed', err));
...
function openMarketSnapshotReportFromPrompt() {
  ...
  return openMarketSnapshotReportAt(s.pendingCenter, s.radiusMiles);
}
...
function openNewDealMarketPreview(deal, radiusMiles) {
  ...
  // Use the exact same Market Preview engine as the sidebar workflow.
  // Open the report first, then close the pin popup so Leaflet cannot dismiss or
  // interfere with the preview click before the modal is visible.
  const opened = openMarketSnapshotReportAt(L.latLng(lat, lng), radius);
  if (opened) setTimeout(() => state.map?.closePopup(), 0);
  return opened;
}
...
function enableMarkerHoverPopup(marker) {
  ...
  marker._atlasMarkerHovering = false;
  marker._atlasPopupHovering = false;
  marker._atlasHoverCloseTimer = null;

  const scheduleHoverClose = () => {
    if (marker._atlasPopupPinned || marker._atlasMarkerHovering || marker._atlasPopupHovering) return;
    if (marker._atlasHoverCloseTimer) clearTimeout(marker._atlasHoverCloseTimer);
    marker._atlasHoverCloseTimer = setTimeout(() => {
      marker._atlasHoverCloseTimer = null;
      if (!marker._atlasPopupPinned && !marker._atlasMarkerHovering && !marker._atlasPopupHovering && marker.getPopup?.()) marker.closePopup();
    }, 120);
  };

  marker.on('mouseover', () => {
    marker._atlasMarkerHovering = true;
    if (marker._atlasHoverCloseTimer) { clearTimeout(marker._atlasHoverCloseTimer); marker._atlasHoverCloseTimer = null; }
    if (!marker._atlasPopupPinned && marker.getPopup?.()) marker.openPopup();
  });
  marker.on('mouseout', () => {
    marker._atlasMarkerHovering = false;
    scheduleHoverClose();
  ...
  marker.on('popupopen', (event) => {
    ...
    if (!popupEl) return;
    // Keep popup controls from bubbling into the Leaflet map and bridge the
    // hover from the marker onto the popup itself.
    L.DomEvent.disableClickPropagation(popupEl);
    L.DomEvent.disableScrollPropagation(popupEl);
    L.DomEvent.on(popupEl, 'mouseenter', () => {
      marker._atlasPopupHovering = true;
      if (marker._atlasHoverCloseTimer) { clearTimeout(marker._atlasHoverCloseTimer); marker._atlasHoverCloseTimer = null; }
    });
    L.DomEvent.on(popupEl, 'mouseleave', () => {
      marker._atlasPopupHovering = false;
      scheduleHoverClose();
    });
  });
