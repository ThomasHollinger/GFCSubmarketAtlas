const CARTO_BASEMAP_API_KEY = 'cb1_2q0x_1_7d06ed9fa43813551f76228a';
...
const state = {
  ...
  newDealsAddCategory: null,
  newDealsAddCategoryChooserOpen: false,
  newDealsFirebaseReady: false,
  newDealsUser: null,
  newDealsUnsubscribe: null,
  newDealsCloudLoaded: false,
  newDealsListExpanded: false,
  newDealFilters: { newDeal: false, coldLead: false },
  healthcareLayer: null,
  addressLayer: null,
  addressesEnabled: false,
  addressCache: new Map(),
  addressTileCache: new Map(),
  addressArchivePromise: null,
  addressArchiveRelease: '',
...
const DIRECTIONS_ROUTER_URL = 'https://router.project-osrm.org/route/v1/driving';
...
const ADDRESS_MIN_ZOOM = 16;
const ADDRESS_QUERY_GRID_DEGREES = 0.01;
const ADDRESS_MAX_MARKERS = 1200;
const ADDRESS_TILE_ZOOM = 14;
const OVERTURE_STAC_URL = 'https://stac.overturemaps.org';
const OVERTURE_ADDRESS_RELEASE_FALLBACK = '2026-08-19.0';
const OVERTURE_ADDRESS_TILES_ROOT = 'https://overturemaps-extras-us-west-2.s3.us-west-2.amazonaws.com/tiles';
...
async function ensureQuickviewDataLoaded() {
  ...
  try {
    const sources = [
      ...
      'data/market_quickview/bay_county_submarket_2_quickview_blocks.geojson',
...
function newDealCategory(deal) {
  const value = String(deal?.category || '').trim().toLowerCase();
  return ['coldlead', 'cold-lead', 'cold_lead', 'cold'].includes(value)
    ? NEW_DEAL_CATEGORY.COLD_LEAD
    : NEW_DEAL_CATEGORY.NEW_DEAL;
}

function newDealCategoryLabel(dealOrCategory) {
  const category = typeof dealOrCategory === 'string'
    ? newDealCategory({ category: dealOrCategory })
    : newDealCategory(dealOrCategory);
  return category === NEW_DEAL_CATEGORY.COLD_LEAD ? 'Cold Lead' : 'New Deal';
}

function newDealCategoryEnabled(deal) {
  return state.newDealFilters[newDealCategory(deal)] !== false;
}

function visiblePrivateDeals() {
  return (state.newDeals || []).filter(newDealCategoryEnabled);
}

function newDealCategoryCount(category) {
  return (state.newDeals || []).filter(deal => newDealCategory(deal) === category).length;
}

function anyNewDealCategoryEnabled() {
  return !!(state.newDealFilters.newDeal || state.newDealFilters.coldLead);
}
...
function clearPrivateNewDealsData() {
  ...
  state.newDealsAddCategory = null;
  state.newDealsAddCategoryChooserOpen = false;
  state.newDealFilters.newDeal = false;
  state.newDealFilters.coldLead = false;
  ['toggleNewDeals', 'toggleColdLeads'].forEach(id => {
...
function newDealIcon(deal) {
  ...
  const markerClass = newDealCategory(deal) === NEW_DEAL_CATEGORY.COLD_LEAD
    ? 'new-deal-marker cold-lead-marker'
    : 'new-deal-marker';
  return L.divIcon({
    className: '',
    html: `<div class="${markerClass}">${initial}</div>`,
    iconSize: [24, 24],
    iconAnchor: [12, 12],
    popupAnchor: [0, -12]
  });
...
function dealsFromGeoJson(collection) {
  return (collection?.features || []).map((feature, index) => {
    ...
    return {
      ...
      category: newDealCategory({ category: p.category })
    };
  }).filter(Boolean);
}
...
function updateNewDealsAuthUI(message = '') {
  ...
  const coldBadge = document.getElementById('coldLeadsCountBadge');
  ...
  if (status) {
    ...
    else if (authorized) status.textContent = 'Private • Deals & Leads unlocked for this session';
    else status.textContent = 'Private • enter the team password to view Deals & Leads';
  }
  if (badge) badge.textContent = authorized ? `${newDealCategoryCount(NEW_DEAL_CATEGORY.NEW_DEAL)} deals` : 'Private';
  if (coldBadge) coldBadge.textContent = authorized ? `${newDealCategoryCount(NEW_DEAL_CATEGORY.COLD_LEAD)} leads` : 'Private';
...
async function focusNewDealFromSidebar(deal) {
  ...
  const category = newDealCategory(deal);
  const toggle = document.getElementById(category === NEW_DEAL_CATEGORY.COLD_LEAD ? 'toggleColdLeads' : 'toggleNewDeals');
  if (toggle) toggle.checked = true;
  state.newDealFilters[category] = true;
  rebuildNewDealsLayer();
...
function renderNewDealsSidebarList() {
  ...
  const deals = visiblePrivateDeals();
  btn.setAttribute('aria-expanded', expanded ? 'true' : 'false');
  btn.textContent = expanded ? `Hide Deals & Leads (${deals.length})` : `Show Deals & Leads (${deals.length})`;
  ...
  if (!deals.length) {
    ...
    empty.textContent = anyNewDealCategoryEnabled() ? 'No pins in the selected category.' : 'Select New Deals and/or Cold Leads to show pins.';
    list.appendChild(empty);
    return;
  }

  [NEW_DEAL_CATEGORY.NEW_DEAL, NEW_DEAL_CATEGORY.COLD_LEAD].forEach(category => {
    const categoryDeals = deals.filter(deal => newDealCategory(deal) === category);
    if (!categoryDeals.length) return;

    const categoryHeading = document.createElement('div');
    categoryHeading.className = 'new-deal-category-heading';
    categoryHeading.textContent = category === NEW_DEAL_CATEGORY.COLD_LEAD
      ? `Cold Leads (${categoryDeals.length})`
      : `New Deals (${categoryDeals.length})`;
    ...
    categoryDeals.forEach(deal => {
      ...
      if (!groups.has(key)) groups.set(key, { number, name: submarketName || 'Outside Submarket Boundary', deals: [] });
    ...
    [...groups.values()].sort((a, b) => {
      ...
      heading.textContent = Number.isFinite(group.number) ? `${group.number}. ${group.name}` : group.name;
      section.appendChild(heading);
      group.deals.forEach(deal => {
        const item = document.createElement('button');
        item.type = 'button';
        item.className = 'new-deal-list-item';
        item.textContent = deal.name || newDealCategoryLabel(deal);
        item.title = deal.city ? `${deal.name || newDealCategoryLabel(deal)} — ${deal.city}` : (deal.name || newDealCategoryLabel(deal));
...
function updateNewDealsUI() {
  const addBtn = document.getElementById('addNewDealPin');
  const addChoice = document.getElementById('newDealAddCategoryChoice');
  if (addBtn) {
    const addActive = !!(state.newDealsAddMode || state.newDealsAddCategoryChooserOpen);
    addBtn.classList.toggle('active', addActive);
    addBtn.textContent = state.newDealsAddMode
      ? `Click Map to Place ${newDealCategoryLabel(state.newDealsAddCategory)}`
      : (state.newDealsAddCategoryChooserOpen ? 'Cancel Add Pin' : 'Add Pin to Map');
  }
  if (addChoice) addChoice.hidden = !state.newDealsAddCategoryChooserOpen;
  const hint = document.getElementById('newDealsHint');
  if (hint) hint.textContent = state.newDealsAddMode
    ? `Click anywhere on the map to place the ${newDealCategoryLabel(state.newDealsAddCategory).toLowerCase()}.`
    : (state.newDealsAddCategoryChooserOpen
      ? 'Choose the type of pin, then click its location on the map.'
      : (newDealAuthorizedUser(state.newDealsUser) ? 'Private Deals & Leads unlocked. Add, move, convert, and delete are enabled.' : 'Deals and leads are private. Enter the team password to view the pins.'));
...
async function setNewDealAddMode(active, category = NEW_DEAL_CATEGORY.NEW_DEAL) {
  if (active && !await authorizeNewDealEditing()) return;
  state.newDealsAddMode = !!active;
  state.newDealsAddCategory = state.newDealsAddMode ? newDealCategory({ category }) : null;
  state.newDealsAddCategoryChooserOpen = false;
  if (state.newDealsAddMode) {
    const isColdLead = state.newDealsAddCategory === NEW_DEAL_CATEGORY.COLD_LEAD;
    const toggle = document.getElementById(isColdLead ? 'toggleColdLeads' : 'toggleNewDeals');
    if (toggle) toggle.checked = true;
    state.newDealFilters[state.newDealsAddCategory] = true;
    rebuildNewDealsLayer();
    ensureNewDealsLayerVisible(true);
...
async function promptForNewDealAddCategory() {
  if (state.newDealsAddMode || state.newDealsAddCategoryChooserOpen) {
    await setNewDealAddMode(false);
    return;
  }
  if (!await authorizeNewDealEditing()) return;
  state.newDealsAddCategoryChooserOpen = true;
  state.newDealsAddCategory = null;
  updateNewDealsUI();
}
...
async function deleteNewDeal(deal) {
  ...
  catch (err) { console.error(err); alert(`${newDealCategoryLabel(deal)} could not be deleted: ${err.message || err}`); }
}
...
async function moveNewDeal(deal) {
  ...
  marker.once('dragend', async () => {
    ...
    const name = deal.name || `this ${newDealCategoryLabel(deal)}`;
    ...
    try {
      ...
      console.error(err);
      marker.setLatLng(original);
      alert(`${newDealCategoryLabel(deal)} could not be moved: ${err.message || err}`);
    }
  });
}

async function changeNewDealCategory(deal) {
  if (!deal?.id || !await authorizeNewDealEditing()) return;
  const current = newDealCategory(deal);
  const next = current === NEW_DEAL_CATEGORY.COLD_LEAD ? NEW_DEAL_CATEGORY.NEW_DEAL : NEW_DEAL_CATEGORY.COLD_LEAD;
  try {
    await firebase.firestore().collection(NEW_DEALS_COLLECTION).doc(deal.id).update({
      category: next,
      updatedAt: new Date().toISOString()
    });
    state.newDealFilters[next] = true;
    const nextToggle = document.getElementById(next === NEW_DEAL_CATEGORY.COLD_LEAD ? 'toggleColdLeads' : 'toggleNewDeals');
    if (nextToggle) nextToggle.checked = true;
    state.map?.closePopup();
  } catch (err) {
    console.error(err);
    alert(`${newDealCategoryLabel(deal)} could not be converted: ${err.message || err}`);
  }
}
...
function newDealPopupContent(deal) {
  ...
  const category = newDealCategory(deal);
  const categoryLabel = newDealCategoryLabel(deal);
  const changeLabel = category === NEW_DEAL_CATEGORY.COLD_LEAD ? 'Change to New Deal' : 'Change to Cold';
  wrap.innerHTML = `<h3>${escapeHtml(deal.name || categoryLabel)}</h3>
    ...
    <div class="new-deal-popup-actions">
      ...
      <button type="button" class="new-deal-category-btn">${changeLabel}</button>
    </div>`;
  ...
  const changeCategory = wrap.querySelector('.new-deal-category-btn');
  if (changeCategory) changeCategory.addEventListener('click', async (event) => {
    event.preventDefault();
    event.stopPropagation();
    await changeNewDealCategory(deal);
  });
  ...
  if (del) {
    del.addEventListener('click', async (event) => {
      event.preventDefault();
      event.stopPropagation();
      if (!confirm(`Delete ${deal.name || `this ${categoryLabel}`} for everyone?`)) return;
...
async function openNewDealEditor(latlng, category = state.newDealsAddCategory || NEW_DEAL_CATEGORY.NEW_DEAL) {
  if (!await authorizeNewDealEditing()) { setNewDealAddMode(false); return; }
  const selectedCategory = newDealCategory({ category });
  const selectedCategoryLabel = newDealCategoryLabel(selectedCategory);
  const wrap = document.createElement('div');
  wrap.className = 'new-deal-editor';
  wrap.innerHTML = `<h3>Add ${selectedCategoryLabel}</h3><label>${selectedCategoryLabel} Name</label>
    <input type="text" class="new-deal-name-input" maxlength="120" placeholder="Enter ${selectedCategoryLabel.toLowerCase()} name">
  ...
  const save=async()=>{
    ...
    const deal={name,lat:latlng.lat,lng:latlng.lng,city:ctx.city,submarket:ctx.submarket,category:selectedCategory,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};
    try {
    ...
    } catch(err) { console.error(err); alert(`${selectedCategoryLabel} could not be saved: ${err.message||err}`); }
  };
  wrap.querySelector('.new-deal-save-btn')?.addEventListener('click',save);
  input?.addEventListener('keydown',e=>{if(e.key==='Enter')save();});
}

function handleNewDealMapClick(latlng) {
  if (state.newDealsAddMode && latlng) openNewDealEditor(latlng, state.newDealsAddCategory);
}
...
function navigateToNewDealCoordinates() {
  ...
  if(state.newDealsAddMode || state.newDealsAddCategoryChooserOpen)setNewDealAddMode(false);
  showNewDealPlaceResultMarker(parsed.lat, parsed.lng, `Coordinates: ${parsed.lat.toFixed(6)}, ${parsed.lng.toFixed(6)}`);
  state.map?.flyTo([parsed.lat,parsed.lng],17,{animate:true,duration:0.9});
}

function exportPrivateDealCategoryKml(category) {
  const normalizedCategory = newDealCategory({ category });
  const isColdLead = normalizedCategory === NEW_DEAL_CATEGORY.COLD_LEAD;
  const categoryName = isColdLead ? 'Cold Leads' : 'New Deals';
  const categorySingular = isColdLead ? 'Cold Lead' : 'New Deal';
  const filename = isColdLead ? 'Cold_Leads.kml' : 'New_Deals.kml';
  const styleId = isColdLead ? 'coldLeadStyle' : 'newDealStyle';
  const iconName = isColdLead ? 'blu-circle.png' : 'orange-circle.png';
  const deals = (state.newDeals || []).filter(deal => newDealCategory(deal) === normalizedCategory);
  if (!deals.length) {
    alert(`There are no ${categoryName} to export.`);
    return;
  }
  ...
  const styleBlocks = `<Style id="${styleId}"><IconStyle><scale>1.0</scale><Icon><href>https://maps.google.com/mapfiles/kml/paddle/${iconName}</href></Icon></IconStyle></Style>`;
  const kml = buildKmlDocument({
    documentName: `Gulf Coast ${categoryName}`,
    ...
    placemarkOptions: feature => {
      ...
      const description = [
        ...
        `<div><b>Category:</b> ${categorySingular}</div>`,
        `<div><b>City:</b> ${escapeHtml(p.city || '-')}</div>`,
        `<div><b>Submarket:</b> ${escapeHtml(p.submarket || 'Outside submarket boundary')}</div>`
      ].join('');
      return { name: p.name || categorySingular, description, styleUrl: `#${styleId}` };
    }
  });
  downloadKml(filename, kml);
}

function exportNewDealsKml() { exportPrivateDealCategoryKml(NEW_DEAL_CATEGORY.NEW_DEAL); }
function exportColdLeadsKml() { exportPrivateDealCategoryKml(NEW_DEAL_CATEGORY.COLD_LEAD); }
...
function buildLifestyleLayer() {
  ...
  activeLifestyleAmenities().forEach(feature => {
    ...
    marker._atlasDirectionsLabel = p.Name || 'Lifestyle Amenity';
...
function directionLabelForMarker(marker) {
  if (!marker) return 'Selected location';
  if (marker._atlasDirectionsLabel) return String(marker._atlasDirectionsLabel);
  const p = marker.feature && marker.feature.properties ? marker.feature.properties : null;
  if (p) return String(p.Subdivision || p.NAME || p.Name || p.Brand || p.DisplayName || 'Selected location');
  return 'Selected location';
}

function ensureDirectionsButton(marker) {
  ...
  if (!content || content.querySelector('.atlas-directions-btn')) return;
  const btn = document.createElement('button');
  btn.type = 'button';
  btn.className = 'atlas-directions-btn';
  btn.textContent = 'Directions to';
  btn.title = 'Start directions from this location';
  btn.addEventListener('click', (event) => {
    event.preventDefault();
    event.stopPropagation();
    if (typeof L !== 'undefined' && L.DomEvent) L.DomEvent.stop(event);
    const ll = marker.getLatLng();
    startDirectionsFrom(ll, directionLabelForMarker(marker));
    state.map?.closePopup();
  });
  const wrap = document.createElement('div');
  wrap.className = 'atlas-directions-actions';
  wrap.appendChild(btn);
  content.appendChild(wrap);
}

function enableDirectionsDestinationOnMarker(marker) {
  if (!marker || marker._atlasDirectionsDestinationEnabled) return marker;
  marker._atlasDirectionsDestinationEnabled = true;
  return marker;
}
...
function enableMarkerHoverPopup(marker) {
  ...
  marker.on('popupopen', () => {
    ensureDirectionsButton(marker);
    requestAnimationFrame(() => requestAnimationFrame(() => ensureDirectionsButton(marker)));
  });
  marker.on('click', (event) => {
    if (state.directions && state.directions.active) {
      if (event && event.originalEvent && typeof L !== 'undefined' && L.DomEvent) L.DomEvent.stop(event.originalEvent);
      handleDirectionsDestination(marker.getLatLng(), directionLabelForMarker(marker));
      return;
    }
  ...
  enableDirectionsDestinationOnMarker(marker);
  return marker;
}

function ensureDirectionsPanel() {
  if (!state.map) return null;
  let panel = document.getElementById('atlasDirectionsPanel');
  if (panel) return panel;
  panel = document.createElement('div');
  panel.id = 'atlasDirectionsPanel';
  panel.className = 'atlas-directions-panel';
  panel.innerHTML = `
    <div class="atlas-directions-head"><div><b>Directions</b><span class="atlas-directions-status"></span></div><button type="button" class="atlas-directions-close" title="End directions">×</button></div>
    <div class="atlas-directions-route-labels"><div><span>From</span><b class="atlas-directions-from">—</b></div><div><span>To</span><b class="atlas-directions-to">Click a destination</b></div></div>
    <div class="atlas-directions-summary" hidden><div><span>Distance</span><b class="atlas-directions-distance">—</b></div><div><span>Drive time</span><b class="atlas-directions-time">—</b></div></div>
    <div class="atlas-directions-loading" hidden>Calculating route…</div>
    <ol class="atlas-directions-steps" hidden></ol>
    <button type="button" class="atlas-directions-end">End Directions</button>`;
  const mapEl = state.map.getContainer();
  mapEl.appendChild(panel);
  panel.querySelector('.atlas-directions-close')?.addEventListener('click', () => endDirections());
  panel.querySelector('.atlas-directions-end')?.addEventListener('click', () => endDirections());
  return panel;
}

function formatDirectionsDistance(meters) {
  const miles = Number(meters || 0) / 1609.344;
  if (miles < 0.1) return `${Math.round(Number(meters || 0) * 3.28084)} ft`;
  return `${miles.toFixed(miles < 10 ? 1 : 0)} mi`;
}

function formatDirectionsDuration(seconds) {
...
function directionsStepText(step) {
  const m = step && step.maneuver ? step.maneuver : {};
  const type = String(m.type || '').toLowerCase();
  const modifier = String(m.modifier || '').toLowerCase().replace(/_/g, ' ');
  const road = step && step.name ? String(step.name) : '';
  if (type === 'depart') return road ? `Depart on ${road}` : 'Depart';
  if (type === 'arrive') return 'Arrive at your destination';
  if (type === 'roundabout' || type === 'rotary') return road ? `Enter the roundabout toward ${road}` : 'Enter the roundabout';
  if (type === 'merge') return road ? `Merge onto ${road}` : 'Merge';
  if (type === 'fork') return road ? `Keep ${modifier || 'ahead'} onto ${road}` : `Keep ${modifier || 'ahead'}`;
  if (type === 'on ramp') return road ? `Take the ramp to ${road}` : 'Take the ramp';
  if (type === 'off ramp') return road ? `Take the exit to ${road}` : 'Take the exit';
  if (type === 'continue') return road ? `Continue on ${road}` : 'Continue straight';
  if (type === 'new name') return road ? `Continue onto ${road}` : 'Continue';
  if (type === 'turn') return road ? `Turn ${modifier || ''} onto ${road}`.replace('  ', ' ').trim() : `Turn ${modifier || ''}`.trim();
  return road ? `Continue on ${road}` : 'Continue';
}

function updateDirectionsPanel(stateText = '') {
  const panel = ensureDirectionsPanel();
  if (!panel) return;
  panel.querySelector('.atlas-directions-status').textContent = stateText ? ` • ${stateText}` : '';
  panel.querySelector('.atlas-directions-from').textContent = state.directions.originLabel || '—';
  panel.querySelector('.atlas-directions-to').textContent = state.directions.destinationLabel || 'Click a destination';
}

function clearDirectionsRouteLayers() {
  if (state.directions.routeLayer && state.map && state.map.hasLayer(state.directions.routeLayer)) state.map.removeLayer(state.directions.routeLayer);
  if (state.directions.destinationMarker && state.map && state.map.hasLayer(state.directions.destinationMarker)) state.map.removeLayer(state.directions.destinationMarker);
  state.directions.routeLayer = null;
  state.directions.destinationMarker = null;
}

function setDirectionsCursor(active) {
  const mapEl = state.map?.getContainer();
  if (mapEl) mapEl.classList.toggle('atlas-directions-active', !!active);
}

function startDirectionsFrom(latlng, label = 'Selected location') {
  if (!state.map || !latlng) return;
  clearDirectionsRouteLayers();
  state.directions.active = true;
  state.directions.origin = { lat: Number(latlng.lat), lng: Number(latlng.lng) };
  state.directions.originLabel = label || 'Selected location';
  state.directions.destination = null;
  state.directions.destinationLabel = '';
  state.directions.requestId += 1;
  const panel = ensureDirectionsPanel();
  panel.hidden = false;
  panel.classList.add('active');
  panel.querySelector('.atlas-directions-summary').hidden = true;
  panel.querySelector('.atlas-directions-loading').hidden = true;
  panel.querySelector('.atlas-directions-steps').hidden = true;
  updateDirectionsPanel('Choose destination');
  setDirectionsCursor(true);
}

async function fetchDrivingRoute(origin, destination, requestId) {
  const coords = `${Number(origin.lng)},${Number(origin.lat)};${Number(destination.lng)},${Number(destination.lat)}`;
  const url = `${DIRECTIONS_ROUTER_URL}/${coords}?overview=full&geometries=geojson&steps=true&alternatives=false`;
  const response = await fetch(url, { headers: { 'Accept': 'application/json' } });
  if (requestId !== state.directions.requestId) return null;
  if (!response.ok) throw new Error(`Routing service returned ${response.status}`);
  const data = await response.json();
  if (!data || data.code !== 'Ok' || !data.routes || !data.routes.length) throw new Error('No driving route was found between those locations.');
  return data.routes[0];
}

async function handleDirectionsDestination(latlng, label = 'Selected location') {
  if (!state.directions.active || !latlng) return;
  const destination = { lat: Number(latlng.lat), lng: Number(latlng.lng) };
  if (!Number.isFinite(destination.lat) || !Number.isFinite(destination.lng)) return;
  state.directions.destination = destination;
  state.directions.destinationLabel = label || 'Selected location';
  state.directions.requestId += 1;
  const requestId = state.directions.requestId;
  clearDirectionsRouteLayers();
  const panel = ensureDirectionsPanel();
  panel.hidden = false;
  panel.classList.add('active');
  panel.querySelector('.atlas-directions-summary').hidden = true;
  panel.querySelector('.atlas-directions-steps').hidden = true;
  panel.querySelector('.atlas-directions-loading').hidden = false;
  updateDirectionsPanel('Calculating route');
  if (state.map) {
    state.directions.destinationMarker = L.circleMarker([destination.lat, destination.lng], { radius: 8, color: '#17385f', weight: 3, fillColor: '#fff', fillOpacity: 1, zIndexOffset: 2000 });
    state.directions.destinationMarker.addTo(state.map);
  }
  try {
    const route = await fetchDrivingRoute(state.directions.origin, destination, requestId);
    if (!route || requestId !== state.directions.requestId || !state.directions.active) return;
    state.directions.routeLayer = L.geoJSON(route.geometry, { style: { color: '#17385f', weight: 5, opacity: 0.9 } }).addTo(state.map);
    const summary = panel.querySelector('.atlas-directions-summary');
    summary.hidden = false;
    panel.querySelector('.atlas-directions-distance').textContent = formatDirectionsDistance(route.distance);
    panel.querySelector('.atlas-directions-time').textContent = formatDirectionsDuration(route.duration);
    const steps = panel.querySelector('.atlas-directions-steps');
    const allSteps = (route.legs || []).flatMap(leg => leg.steps || []).filter(Boolean);
    steps.innerHTML = allSteps.slice(0, 30).map(step => `<li><div>${escapeHtml(directionsStepText(step))}</div><span>${formatDirectionsDistance(step.distance || 0)}</span></li>`).join('');
    steps.hidden = !allSteps.length;
    panel.querySelector('.atlas-directions-loading').hidden = true;
    updateDirectionsPanel('Route ready');
    if (state.map && state.directions.routeLayer) {
      const bounds = state.directions.routeLayer.getBounds();
      if (bounds && bounds.isValid()) state.map.fitBounds(bounds, { padding: [56, 56], maxZoom: 15 });
    }
  } catch (err) {
    if (requestId !== state.directions.requestId || !state.directions.active) return;
    panel.querySelector('.atlas-directions-loading').hidden = true;
    panel.querySelector('.atlas-directions-summary').hidden = true;
    const steps = panel.querySelector('.atlas-directions-steps');
    steps.hidden = false;
    steps.innerHTML = `<li class="atlas-directions-error">${escapeHtml(err.message || 'Could not calculate driving directions.')}</li>`;
    updateDirectionsPanel('Route unavailable');
  }
}

function handleDirectionsMapClick(latlng) {
  if (!state.directions.active) return false;
  handleDirectionsDestination(latlng, 'Map location');
  return true;
}
...
function endDirections() {
  state.directions.requestId += 1;
  clearDirectionsRouteLayers();
  state.directions.active = false;
  state.directions.origin = null;
  state.directions.destination = null;
  state.directions.originLabel = '';
  state.directions.destinationLabel = '';
  setDirectionsCursor(false);
  const panel = document.getElementById('atlasDirectionsPanel');
...
function builderSubdivisionPopupContent(feature) {
  ...
  wrap.innerHTML = `
    ...
    ${previewReady ? `
      ...
      <div class="builder-directions-actions"><button type="button" class="atlas-directions-btn" data-builder-directions>Directions to</button></div>` : `<div class="builder-directions-actions"><button type="button" class="atlas-directions-btn" data-builder-directions>Directions to</button></div>`}`;

  const builderDirectionsBtn = wrap.querySelector('[data-builder-directions]');
  if (builderDirectionsBtn && previewReady) {
    builderDirectionsBtn.addEventListener('click', (event) => {
      event.preventDefault();
      event.stopPropagation();
      if (typeof L !== 'undefined' && L.DomEvent) L.DomEvent.stop(event);
      startDirectionsFrom({ lat, lng }, p.Subdivision || 'Builder Subdivision');
      state.map?.closePopup();
    });
  }
...
function buildBuilderLayer() {
  ...
  activeBuilderSubdivisions().forEach(feature => {
    ...
    marker.feature = feature;
    marker._atlasDirectionsLabel = p.Subdivision || 'Builder Subdivision';
...
function setAddressBadge(text) {
  const badge = document.getElementById('addressCountBadge');
  if (badge) badge.textContent = text;
}

function ensureAddressLayer() {
  if (state.addressLayer) return state.addressLayer;
  state.addressLayer = L.layerGroup();
  state.addressLayer.getAttribution = () => '<a href="https://docs.overturemaps.org/attribution" target="_blank">&copy; Overture Maps Foundation</a>, <a href="https://www.openstreetmap.org/copyright" target="_blank">&copy; OpenStreetMap</a>';
  return state.addressLayer;
}

function alignedAddressQueryBounds() {
  const raw = state.map.getBounds().pad(0.18);
  const step = ADDRESS_QUERY_GRID_DEGREES;
  const south = Math.floor(raw.getSouth() / step) * step;
  const west = Math.floor(raw.getWest() / step) * step;
  const north = Math.ceil(raw.getNorth() / step) * step;
  const east = Math.ceil(raw.getEast() / step) * step;
  return { south, west, north, east, key: [south, west, north, east].map(value => value.toFixed(4)).join('|') };
}

function addressOverpassQuery(bounds) {
  return `[out:json][timeout:25];nwr["addr:housenumber"](${bounds.south},${bounds.west},${bounds.north},${bounds.east});out center tags;`;
}

function addressRecordFromElement(element) {
  ...
  if (!Number.isFinite(Number(lon)) || !Number.isFinite(Number(lat)) || !houseNumber) return null;
  const street = String(tags['addr:street'] || tags['addr:place'] || '').trim();
  const unit = String(tags['addr:unit'] || '').trim();
  const city = String(tags['addr:city'] || tags['addr:town'] || tags['addr:village'] || tags['addr:hamlet'] || '').trim();
  const stateName = String(tags['addr:state'] || tags['addr:province'] || '').trim();
  const postcode = String(tags['addr:postcode'] || '').trim();
  const lineOne = [houseNumber, street].filter(Boolean).join(' ');
  const locality = [city, stateName, postcode].filter(Boolean).join(', ').replace(/, ([0-9]{5}(?:-[0-9]{4})?)$/, ' $1');
  const fullAddress = [lineOne || `House number ${houseNumber}`, unit ? `Unit ${unit}` : '', locality].filter(Boolean).join(', ');
  const dedupeKey = street
    ? [houseNumber, street, city, postcode].map(value => value.toLowerCase()).join('|')
    : [houseNumber, Number(lat).toFixed(5), Number(lon).toFixed(5)].join('|');
  return { id: `${element.type}/${element.id}`, lat: Number(lat), lon: Number(lon), houseNumber, fullAddress, dedupeKey };
}

function addressTileX(lon, zoom = ADDRESS_TILE_ZOOM) {
  const scale = 2 ** zoom;
  return Math.max(0, Math.min(scale - 1, Math.floor(((Number(lon) + 180) / 360) * scale)));
}

function addressTileY(lat, zoom = ADDRESS_TILE_ZOOM) {
  const scale = 2 ** zoom;
  const limitedLat = Math.max(-85.05112878, Math.min(85.05112878, Number(lat)));
  const radians = limitedLat * Math.PI / 180;
  const value = Math.floor((1 - Math.asinh(Math.tan(radians)) / Math.PI) / 2 * scale);
  return Math.max(0, Math.min(scale - 1, value));
}

function addressTilesForBounds(bounds) {
  const minX = addressTileX(bounds.west);
  const maxX = addressTileX(bounds.east);
  const minY = addressTileY(bounds.north);
  const maxY = addressTileY(bounds.south);
  const tiles = [];
  for (let x = minX; x <= maxX; x += 1) {
    for (let y = minY; y <= maxY; y += 1) tiles.push({ z: ADDRESS_TILE_ZOOM, x, y });
  }
  return tiles;
}

function overtureAddressState(properties = {}) {
  try {
    const levels = JSON.parse(properties.address_levels || '[]');
    const values = Array.isArray(levels) ? levels.map(level => String(level?.value || '').trim()).filter(Boolean) : [];
    return values.find(value => /^[A-Z]{2}$/.test(value)) || '';
  } catch (err) {
    return '';
  }
}

function addressRecordFromOvertureFeature(feature, tile) {
  const geometry = feature.toGeoJSON(tile.x, tile.y, tile.z)?.geometry;
  const coordinates = geometry?.coordinates;
  const properties = feature.properties || {};
  const houseNumber = String(properties.number || '').trim();
  if (geometry?.type !== 'Point' || !Array.isArray(coordinates) || !houseNumber) return null;
  const lon = Number(coordinates[0]);
  const lat = Number(coordinates[1]);
  if (!Number.isFinite(lon) || !Number.isFinite(lat)) return null;
  const street = String(properties.street || '').trim();
  const unit = String(properties.unit || '').trim();
  const city = String(properties.postal_city || '').trim().toLowerCase().replace(/\b\w/g, letter => letter.toUpperCase());
  const stateName = overtureAddressState(properties);
  ...
  const dedupeKey = street
    ? [houseNumber, street, city, postcode].map(value => value.toLowerCase()).join('|')
    : [houseNumber, lat.toFixed(5), lon.toFixed(5)].join('|');
  return { id: String(properties.id || `${tile.z}/${tile.x}/${tile.y}/${feature.id}`), lat, lon, houseNumber, fullAddress, dedupeKey };
}

async function ensureOvertureAddressArchive() {
  if (state.addressArchivePromise) return state.addressArchivePromise;
  state.addressArchivePromise = (async () => {
    if (!window.AtlasAddressTiles?.PMTiles) throw new Error('Address tile reader is unavailable');
    let release = OVERTURE_ADDRESS_RELEASE_FALLBACK;
    try {
      const response = await fetch(OVERTURE_STAC_URL, { headers: { Accept: 'application/json' } });
      if (response.ok) {
        const catalog = await response.json();
        if (/^\d{4}-\d{2}-\d{2}\.\d+$/.test(String(catalog.latest || ''))) release = catalog.latest;
      }
    } catch (err) {
      console.warn('Could not check the latest Overture address release; using the packaged fallback release.', err);
    }
    const url = `${OVERTURE_ADDRESS_TILES_ROOT}/${encodeURIComponent(release)}/addresses.pmtiles`;
    const archive = new window.AtlasAddressTiles.PMTiles(url);
    await archive.getHeader();
    state.addressArchiveRelease = release;
    return archive;
  })();
  try {
    return await state.addressArchivePromise;
  } catch (err) {
    state.addressArchivePromise = null;
    throw err;
  }
}

async function loadOvertureAddressTile(tile) {
  const key = `${tile.z}/${tile.x}/${tile.y}`;
  if (state.addressTileCache.has(key)) return state.addressTileCache.get(key);
  const loading = (async () => {
    const archive = await ensureOvertureAddressArchive();
    const result = await archive.getZxy(tile.z, tile.x, tile.y);
    if (!result?.data) return [];
    const { VectorTile, PbfReader } = window.AtlasAddressTiles || {};
    if (!VectorTile || !PbfReader) throw new Error('Address vector-tile decoder is unavailable');
    const vectorTile = new VectorTile(new PbfReader(result.data));
    const layer = vectorTile.layers?.address;
    if (!layer) return [];
    const records = [];
    for (let index = 0; index < layer.length; index += 1) {
      const record = addressRecordFromOvertureFeature(layer.feature(index), tile);
      if (record) records.push(record);
    }
    return records;
  })();
  state.addressTileCache.set(key, loading);
  try {
    return await loading;
  } catch (err) {
    state.addressTileCache.delete(key);
    throw err;
  }
}

async function loadResidentialAddressRecords(bounds) {
  try {
    const groups = await Promise.all(addressTilesForBounds(bounds).map(loadOvertureAddressTile));
    return groups.flat().filter(record => record.lat >= bounds.south && record.lat <= bounds.north && record.lon >= bounds.west && record.lon <= bounds.east);
  } catch (err) {
    console.warn('Broader address-point source unavailable; falling back to OpenStreetMap addresses.', err);
    const data = await fetchOverpass(addressOverpassQuery(bounds));
    return (data.elements || []).map(addressRecordFromElement).filter(Boolean);
  }
}
...
function bindAddressCopyPopup(marker, record) {
  const coordinates = `${record.lat.toFixed(6)}, ${record.lon.toFixed(6)}`;
  marker._atlasDirectionsLabel = record.fullAddress;
  marker.bindPopup(`
    <div class="address-copy-popup">
      <div class="address-copy-value">${escapeHtml(record.fullAddress)}</div>
      <div class="address-copy-actions">
        <button type="button" class="address-copy-btn" data-copy-kind="address">Copy Address</button>
        <button type="button" class="address-copy-btn" data-copy-kind="coordinates">Copy Coordinates</button>
      </div>
      <div class="address-copy-status" aria-live="polite"></div>
    </div>
  `, {
    autoClose: true,
    autoPan: false,
    closeButton: true,
    closeOnClick: true,
    className: 'address-copy-popup-shell'
  });
  marker.on('popupopen', event => {
    ...
    const addressButton = popupElement?.querySelector('[data-copy-kind="address"]');
    const coordinatesButton = popupElement?.querySelector('[data-copy-kind="coordinates"]');
    const status = popupElement?.querySelector('.address-copy-status');
    const closeOnMove = () => marker.closePopup();
    const closeOnEscape = keyEvent => {
      if (keyEvent.key === 'Escape') marker.closePopup();
    };
    state.map.once('movestart', closeOnMove);
    document.addEventListener('keydown', closeOnEscape);
    marker.once('popupclose', () => {
      state.map.off('movestart', closeOnMove);
      document.removeEventListener('keydown', closeOnEscape);
    });
    const wireCopyButton = (button, value, successMessage) => {
      if (!button) return;
      button.onclick = async clickEvent => {
        ...
        try {
          ...
          console.warn('Could not copy map information to clipboard', err);
          button.textContent = 'Copy failed';
          if (status) status.textContent = 'Clipboard access was unavailable.';
        }
      };
    };
    wireCopyButton(addressButton, record.fullAddress, 'Address copied to clipboard.');
    wireCopyButton(coordinatesButton, coordinates, 'Coordinates copied to clipboard.');
    ensureDirectionsButton(marker);
    requestAnimationFrame(() => ensureDirectionsButton(marker));
  });
  marker.on('click', event => {
    if (!state.directions || !state.directions.active) return;
    if (event?.originalEvent && typeof L !== 'undefined' && L.DomEvent) L.DomEvent.stop(event.originalEvent);
    marker.closePopup();
    handleDirectionsDestination(marker.getLatLng(), record.fullAddress);
  });
}

function renderAddressRecords(records) {
  const layer = ensureAddressLayer();
  layer.clearLayers();
  if (!records.length) {
    setAddressBadge('No addresses');
    return;
  }
  if (records.length > ADDRESS_MAX_MARKERS) {
    setAddressBadge('Zoom closer');
    return;
  }
  const seen = new Set();
  records.forEach(record => {
    if (!record || seen.has(record.dedupeKey)) return;
    seen.add(record.dedupeKey);
    const width = Math.max(22, Math.min(64, 12 + record.houseNumber.length * 7));
    const icon = L.divIcon({
      className: 'address-number-label',
      html: `<span>${escapeHtml(record.houseNumber)}</span>`,
      iconSize: [width, 18],
      iconAnchor: [width / 2, 9]
    });
    const marker = L.marker([record.lat, record.lon], {
      icon,
      pane: 'addressLabelPane',
      keyboard: false,
      bubblingMouseEvents: false,
      zIndexOffset: -200
    });
    marker.bindTooltip(escapeHtml(record.fullAddress), {
      direction: 'top',
      offset: [0, -7],
      opacity: 0.98,
      className: 'address-full-tooltip'
    });
    bindAddressCopyPopup(marker, record);
    layer.addLayer(marker);
  });
  setAddressBadge(`${seen.size.toLocaleString()} shown`);
}

async function refreshAddressLayer() {
  if (!state.addressesEnabled || !state.map) return;
  const layer = ensureAddressLayer();
  if (!state.map.hasLayer(layer)) layer.addTo(state.map);
  if (state.map.getZoom() < ADDRESS_MIN_ZOOM) {
    state.addressRequestId += 1;
    state.addressLastQueryKey = '';
    layer.clearLayers();
    setAddressBadge('Zoom');
    return;
  }
  const bounds = alignedAddressQueryBounds();
  if (bounds.key === state.addressLastQueryKey && layer.getLayers().length) return;
  state.addressLastQueryKey = bounds.key;
  const cached = state.addressCache.get(bounds.key);
  if (cached) {
    renderAddressRecords(cached);
    return;
  }
  const requestId = ++state.addressRequestId;
  setAddressBadge('Loading...');
  try {
    const records = await loadResidentialAddressRecords(bounds);
    if (!state.addressesEnabled || requestId !== state.addressRequestId) return;
    state.addressCache.set(bounds.key, records);
    renderAddressRecords(records);
    if (!records.length) setAddressBadge('No addresses');
  } catch (err) {
    if (!state.addressesEnabled || requestId !== state.addressRequestId) return;
    console.warn('Address layer could not be loaded', err);
    state.addressLastQueryKey = '';
    layer.clearLayers();
    setAddressBadge('Unavailable');
  }
}

function scheduleAddressRefresh() {
  if (!state.addressesEnabled) return;
  clearTimeout(state.addressRefreshTimer);
  state.addressRefreshTimer = setTimeout(() => refreshAddressLayer(), 280);
}

function setAddressesEnabled(enabled) {
  state.addressesEnabled = !!enabled;
  clearTimeout(state.addressRefreshTimer);
  if (!state.addressesEnabled) {
    state.addressRequestId += 1;
    state.addressLastQueryKey = '';
    if (state.addressLayer && state.map.hasLayer(state.addressLayer)) state.map.removeLayer(state.addressLayer);
    setAddressBadge('Zoom');
    return;
  }
  ensureAddressLayer().addTo(state.map);
...
function buildPOILayer() {
  ...
  activeRetailPOIs().forEach(feature => {
    ...
    marker.feature = feature;
    marker._atlasDirectionsLabel = p.Name || p.Brand || 'Retail / Dining';
...
async function loadHealthcare(showLayer = false) {
  if (!state.healthcareLoaded) {
    ...
    state.healthcareLayer = L.geoJSON({ type: 'FeatureCollection', features: state.healthcare }, {
      pointToLayer: (feature, latlng) => { const marker = L.marker(latlng, { icon: healthcareIcon(feature.properties.FacilityType) }); marker.feature = feature; marker._atlasDirectionsLabel = feature.properties?.Name || 'Healthcare Facility'; return marker; },
...
function initMap() {
  ...
  state.basemaps.light = L.tileLayer(`https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png?key=${encodeURIComponent(CARTO_BASEMAP_API_KEY)}`, {
    attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>, &copy; <a href="https://carto.com/attributions">CARTO</a>',
    subdomains: 'abcd',
    maxZoom: 20
  }).addTo(state.map);
  state.basemaps.streets = L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
    attribution: '&copy; OpenStreetMap contributors', maxNativeZoom: 19, maxZoom: 20
  });
  state.basemaps.topo = L.tileLayer('https://{s}.tile.opentopomap.org/{z}/{x}/{y}.png', {
    attribution: '&copy; OpenTopoMap contributors', maxNativeZoom: 17, maxZoom: 20
  });
  ...
  state.map.createPane('addressLabelPane');
  state.map.getPane('addressLabelPane').style.zIndex = 590;
  state.map.getPane('addressLabelPane').style.pointerEvents = 'auto';
...
async function loadData() {
  const [geojson, meta, demographics, demographicsBlockGroups, quickviewLegacy, centralQuickviewBlocks, centralBaldwinQuickviewBlocks, westQuickviewBlocks, southQuickviewBlocks, northBaldwinQuickviewBlocks, southBaldwinQuickviewBlocks, pensacolaQuickviewBlocks, cantonmentQuickviewBlocks, paceQuickviewBlocks, miltonQuickviewBlocks, pensacolaBeachesQuickviewBlocks, fortWaltonQuickviewBlocks, crestviewQuickviewBlocks, laurelHillQuickviewBlocks, waltonBayBeachesQuickviewBlocks, freeportQuickviewBlocks, defuniakSpringsQuickviewBlocks, panamaCityQuickviewBlocks, bayCountySubmarket2QuickviewBlocks, mariannaQuickviewBlocks, healthcareFacilities, healthcareSummary] = await Promise.all([
    ...
    fetch('data/market_quickview/bay_county_submarket_2_quickview_blocks.geojson').then(r => r.json()).catch(() => ({ type: 'FeatureCollection', features: [] })),
  ...
  const quickviewCombinedFeatures = [
    ...
    ...(bayCountySubmarket2QuickviewBlocks?.features || []),
  ...
  state.submarketLayer = L.geoJSON(geojson, {
    ...
    onEachFeature: (feature, layer) => {
      layer.on({
        ...
        click: (e) => { if (state.directions && state.directions.active) { if (e && e.originalEvent) L.DomEvent.stop(e.originalEvent); handleDirectionsMapClick(e.latlng || layer.getCenter?.() || null); return; } if (state.newDealsAddMode) { if (e && e.originalEvent) L.DomEvent.stop(e.originalEvent); handleNewDealMapClick(e.latlng || layer.getCenter?.() || null); return; } if (marketSnapshotModeActive()) { if (e && e.originalEvent) L.DomEvent.stop(e.originalEvent); handleMarketSnapshotPoint(e.latlng || layer.getCenter?.() || null); return; } selectFeature(feature, layer, false); },
        dblclick: (e) => { if (state.directions && state.directions.active) { if (e && e.originalEvent) L.DomEvent.stop(e.originalEvent); return; } if (state.newDealsAddMode) { if (e && e.originalEvent) L.DomEvent.stop(e.originalEvent); return; } if (marketSnapshotModeActive()) { if (e && e.originalEvent) L.DomEvent.stop(e.originalEvent); return; } selectFeature(feature, layer, true); }
      });
      const p = feature.properties;
  ...
  state.map.on('click', e => {
    if (state.directions && state.directions.active) { handleDirectionsMapClick(e.latlng); return; }
    clearNewDealPlaceResultMarker();
...
function refreshSchoolLayer() {
  ...
  state.schoolLayer = L.geoJSON({ type: 'FeatureCollection', features: visibleSchools }, {
    pointToLayer: (feature, latlng) => { const marker = L.marker(latlng, { icon: schoolIcon(feature.properties.SchoolType) }); marker.feature = feature; marker._atlasDirectionsLabel = feature.properties?.NAME || 'School'; return marker; },
    onEachFeature: (feature, layer) => {
      const p = feature.properties;
...
function bindUI() {
  ...
  document.getElementById('toggleAddresses')?.addEventListener('change', e => {
    setAddressesEnabled(e.target.checked);
  });
  state.map.on('moveend zoomend', scheduleAddressRefresh);
  ...
  const handlePrivateDealCategoryToggle = async (e, category) => {
    state.newDealFilters[category] = !!e.target.checked;
    if (e.target.checked) {
      const ok = await authorizeNewDealEditing();
      if (!ok) {
        e.target.checked = false;
        state.newDealFilters[category] = false;
        rebuildNewDealsLayer();
        updateNewDealsUI();
        return;
      }
    ...
    if (category === state.newDealsAddCategory && !e.target.checked && state.newDealsAddMode) await setNewDealAddMode(false);
    rebuildNewDealsLayer();
    if (!anyNewDealCategoryEnabled()) state.newDealsListExpanded = false;
    updateNewDealsUI();
  };
  document.getElementById('toggleNewDeals')?.addEventListener('change', e => handlePrivateDealCategoryToggle(e, NEW_DEAL_CATEGORY.NEW_DEAL));
  document.getElementById('toggleColdLeads')?.addEventListener('change', e => handlePrivateDealCategoryToggle(e, NEW_DEAL_CATEGORY.COLD_LEAD));
  document.getElementById('toggleNewDealsList')?.addEventListener('click', async () => {
    if (!newDealAuthorizedUser(state.newDealsUser)) {
      if (!(await authorizeNewDealEditing())) return;
      subscribeToPrivateNewDeals();
    }
    if (!anyNewDealCategoryEnabled()) {
      state.newDealFilters.newDeal = true;
      state.newDealFilters.coldLead = true;
      const newDealsToggle = document.getElementById('toggleNewDeals');
      const coldLeadsToggle = document.getElementById('toggleColdLeads');
      if (newDealsToggle) newDealsToggle.checked = true;
      if (coldLeadsToggle) coldLeadsToggle.checked = true;
      rebuildNewDealsLayer();
    }
  ...
  document.getElementById('addNewDealPin')?.addEventListener('click', () => {
    promptForNewDealAddCategory();
  });
  document.querySelectorAll('[data-deal-add-category]').forEach(btn => {
    btn.addEventListener('click', () => {
      setNewDealAddMode(true, btn.dataset.dealAddCategory);
    });
  });
  ...
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape') {
      if (state.directions && state.directions.active) endDirections();
      if (state.newDealsAddMode || state.newDealsAddCategoryChooserOpen) setNewDealAddMode(false);
      clearNewDealPlaceResultMarker();
      clearNewDealPlaceSuggestions();
      if (marketSnapshotModeActive()) resetMarketSnapshotMode();
  ...
  document.getElementById('downloadColdLeadsKml')?.addEventListener('click', exportColdLeadsKml);
