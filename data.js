...
function buildBuilderLayer() {
  ...
  activeBuilderSubdivisions().forEach(feature => {
    ...
    marker.bindPopup(`<div class="builder-popup"><h3>${p.Subdivision || 'Builder Community'}</h3><p><b>Builder:</b> ${displayBuilder || '—'}</p><p><b>Status:</b> ${p.Status || '—'}</p><p><b>Product:</b> ${p.ProductStyle || '—'}</p><p><b>Price:</b> ${builderRangeText(p.PriceMin, p.PriceMax, 'money')}</p><p><b>Square Foot:</b> ${builderRangeText(p.UnitSizeMin, p.UnitSizeMax)}</p><p><b>Elementary School:</b> ${builderSchoolText(p.SchoolElementary)}</p><p><b>Middle School:</b> ${builderSchoolText(p.SchoolMiddle)}</p><p><b>High School:</b> ${builderSchoolText(p.SchoolHigh)}</p><p><b>Units Remaining:</b> ${fmt(p.UnitsRemaining)}</p><p><b>Annual Starts:</b> ${fmt(p.AnnualStarts)}</p><p><b>City:</b> ${p.City || ''}, ${p.State || ''}</p><p><b>Submarket:</b> ${p.SubmarketName || 'Outside submarket boundary'}</p><p><b>Tier:</b> ${builderTierForFeature(feature) ? (state.builderTierConfig[builderTierForFeature(feature)] || {}).label || builderTierForFeature(feature) : '—'}</p><p><b>Source:</b> ${p.Source || 'Zonda export'}</p></div>`);
