...
function focusNewDealFromSidebar(deal) {
...
function renderNewDealsSidebarList() {
  ...
  orderedGroups.forEach(group => {
    ...
    group.deals.forEach(deal => {
      ...
      item.addEventListener('click', () => focusNewDealFromSidebar(deal));
...
function openNewDealMarketPreview(deal, radiusMiles) {
  ...
  if (!Number.isFinite(lat) || !Number.isFinite(lng) || ![3, 5].includes(radius)) return;

  // Close the pin popup first and open the existing merged Market Preview directly
  // at this deal's coordinates without entering map-click preview mode.
  state.map?.closePopup();
  state.marketSnapshot = state.marketSnapshot || { active: false, radiusMiles: null, awaitingPoint: false, busy: false, pendingCenter: null, radiusLayer: null, radiusCircle: null, promptMarker: null };
  clearRadiusPromptOverlay('snapshot');
  state.marketSnapshot.active = false;
  ...
  state.marketSnapshot.pendingCenter = L.latLng(lat, lng);
  updateMarketSnapshotUI();
  openMarketSnapshotReportFromPrompt();
}
...
function bindUI() {
  ...
  document.getElementById('toggleNewDealsList')?.addEventListener('click', () => {
