...
function ensureAddressLayer() {
  ...
  state.addressLayer.getAttribution = () => '&copy; OpenStreetMap contributors';
...
async function refreshAddressLayer() {
  ...
  if (state.map.getZoom() < ADDRESS_MIN_ZOOM) {
    ...
    layer.clearLayers();
    setAddressBadge(`Zoom to ${ADDRESS_MIN_ZOOM}+`);
    return;
  }
  ...
  try {
    ...
    const records = (data.elements || []).map(addressRecordFromElement).filter(Boolean);
