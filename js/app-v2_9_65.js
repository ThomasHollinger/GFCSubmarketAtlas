...
function radiusPromptLabel(mode) {
  return mode === 'quickview' ? 'See Market Quickview?' : 'Open Market Preview?';
}
...
function renderRadiusPromptHtml(mode) {
  const yesLabel = mode === 'quickview' ? 'Open Quickview' : 'Open Preview';
  const noLabel = 'No';
...
function openMarketSnapshotReportFromPrompt() {
  ...
  // Open immediately using everything already in memory, then refresh once any
  // background data finishes. This preserves the fast Preview interaction while
  // ensuring Schools / Competition / Retail / Lifestyle populate when ready.
  openMarketSnapshotModal('Market Preview', subtitle, buildMarketPreviewHtml(center, s.radiusMiles));
  const refreshPreview = () => {
    const overlay = document.getElementById('marketSnapshotModal');
    const body = document.getElementById('marketSnapshotModalBody');
    if (overlay?.classList.contains('active') && body) body.innerHTML = buildMarketPreviewHtml(center, s.radiusMiles);
  };
  // Refresh Mean Income as soon as the ACS B19025 cache is ready instead of
  // waiting on slower Retail/Lifestyle background calls.
  ensureAcsMeanIncomeLoaded().then(refreshPreview);
  Promise.allSettled([ensureQuickviewDataLoaded(), ensureSnapshotDataLoaded()]).then(refreshPreview);
}
...
function newDealIcon() {
  return L.divIcon({
    className: '',
    html: '<div class="new-deal-marker">D</div>',
    iconSize: [24, 24],
    iconAnchor: [12, 12],
    popupAnchor: [0, -12]
  });
...
function loadNewDealsFromStorage() {
  try {
    const raw = localStorage.getItem(NEW_DEALS_STORAGE_KEY);
    const parsed = raw ? JSON.parse(raw) : [];
    state.newDeals = Array.isArray(parsed) ? parsed.filter(d => Number.isFinite(Number(d.lat)) && Number.isFinite(Number(d.lng))) : [];
  } catch (err) {
    console.warn('New Deals storage could not be read', err);
    state.newDeals = [];
  }
  updateNewDealsUI();
}

function persistNewDeals() {
  try {
    localStorage.setItem(NEW_DEALS_STORAGE_KEY, JSON.stringify(state.newDeals || []));
  } catch (err) {
    console.warn('New Deals storage could not be saved', err);
    alert('This browser could not save the New Deal pin locally.');
  }
  updateNewDealsUI();
}
...
function updateNewDealsUI() {
  ...
  if (hint) hint.textContent = state.newDealsAddMode ? 'Click anywhere on the map to place the new deal.' : 'Pins are saved in this browser until deleted.';
  if (state.map) state.map.getContainer().classList.toggle('new-deal-add-mode', !!state.newDealsAddMode);
}

function setNewDealAddMode(active) {
  ...
  if (state.newDealsAddMode) {
    ...
    // Builder data gives us a strong local city reference without a live geocoder.
...
function newDealLocationContext(latlng) {
  ...
  (state.builders || []).forEach(f => {
    const c = f?.geometry?.coordinates || [];
    consider(c[1], c[0], f?.properties?.City);
  });
  (state.healthcare || []).forEach(f => {
    const c = f?.geometry?.coordinates || [];
    consider(c[1], c[0], f?.properties?.City);
  });
  (state.schools || []).forEach(f => {
    const c = f?.geometry?.coordinates || [];
    consider(c[1], c[0], f?.properties?.CITY);
  });
  // A nearby known Atlas record is useful for city labeling; farther away, leave it blank.
  const city = nearestDistance <= 12 ? nearestCity : '';
  return { city, submarket };
}
...
function newDealPopupContent(deal) {
  ...
  wrap.innerHTML = `<h3>${escapeHtml(deal.name || 'New Deal')}</h3>
    ...
    <p><b>City:</b> ${cityText}</p>
    <p><b>Submarket:</b> ${submarketText}</p>
    <button type="button" class="new-deal-delete-btn">Delete Pin</button>`;
  wrap.querySelector('.new-deal-delete-btn')?.addEventListener('click', () => {
    if (!confirm(`Delete ${deal.name || 'this New Deal'}?`)) return;
    state.newDeals = (state.newDeals || []).filter(d => d.id !== deal.id);
    persistNewDeals();
...
function rebuildNewDealsLayer() {
  if (!state.map) return;
  const wasVisible = document.getElementById('toggleNewDeals')?.checked;
  if (state.newDealsLayer && state.map.hasLayer(state.newDealsLayer)) state.map.removeLayer(state.newDealsLayer);
  const markers = (state.newDeals || []).map(deal => {
    const marker = L.marker([Number(deal.lat), Number(deal.lng)], { icon: newDealIcon(), zIndexOffset: 1200 });
    marker.bindPopup(() => newDealPopupContent(deal), { minWidth: 220 });
    return marker;
  });
  state.newDealsLayer = L.layerGroup(markers);
...
function ensureNewDealsLayerVisible(visible) {
  ...
  if (visible) {
    if (!state.map.hasLayer(state.newDealsLayer)) state.newDealsLayer.addTo(state.map);
  } else if (state.map.hasLayer(state.newDealsLayer)) {
    state.map.removeLayer(state.newDealsLayer);
  }
}

function openNewDealEditor(latlng) {
  const wrap = document.createElement('div');
  wrap.className = 'new-deal-editor';
  wrap.innerHTML = `<h3>Add New Deal</h3>
    ...
    <div class="new-deal-editor-actions">
      <button type="button" class="new-deal-cancel-btn">Cancel</button>
      <button type="button" class="new-deal-save-btn">Save Pin</button>
    </div>`;
  const popup = L.popup({ closeButton: true, autoClose: true, closeOnClick: false, minWidth: 240 })
    .setLatLng(latlng)
    .setContent(wrap)
    .openOn(state.map);
  const input = wrap.querySelector('.new-deal-name-input');
  ...
  const save = async () => {
    ...
    if (!state.buildersLoaded) {
      try { await loadBuilders(false); } catch (_) {}
    }
    ...
    state.newDeals.push({ id, name, lat: latlng.lat, lng: latlng.lng, city: ctx.city, submarket: ctx.submarket, createdAt: new Date().toISOString() });
    persistNewDeals();
    rebuildNewDealsLayer();
    state.map.closePopup(popup);
    setNewDealAddMode(false);
    const marker = state.newDealsLayer?.getLayers?.().find(m => {
      const ll = m.getLatLng?.();
      return ll && Math.abs(ll.lat - latlng.lat) < 1e-9 && Math.abs(ll.lng - latlng.lng) < 1e-9;
    });
    marker?.openPopup();
  };
...
function handleNewDealMapClick(latlng) {
  if (!state.newDealsAddMode || !latlng) return;
  openNewDealEditor(latlng);
}
...
function parseNewDealCoordinateSearch(rawValue) {
  ...
  // Accept decimal pairs too: 30.620814, -88.281636 or 30.620814 -88.281636.
  ...
  // DMS examples: 30°37'14.93"N 88°16'53.89"W. Degree/minute/second symbols and spacing are flexible.
  ...
  while ((m = re.exec(normalized)) !== null) {
    ...
    const hemi = m[4];
    if (![deg, min, sec].every(Number.isFinite) || min >= 60 || sec >= 60) return null;
    let value = deg + min / 60 + sec / 3600;
    if (hemi === 'S' || hemi === 'W') value *= -1;
    parts.push({ value, hemi });
  }
  if (parts.length !== 2) return null;
  const latPart = parts.find(p => p.hemi === 'N' || p.hemi === 'S');
  const lngPart = parts.find(p => p.hemi === 'E' || p.hemi === 'W');
  if (!latPart || !lngPart || Math.abs(latPart.value) > 90 || Math.abs(lngPart.value) > 180) return null;
  return { lat: latPart.value, lng: lngPart.value };
}

function navigateToNewDealCoordinates() {
  const input = document.getElementById('newDealCoordinateSearch');
  const status = document.getElementById('newDealCoordinateSearchStatus');
  const parsed = parseNewDealCoordinateSearch(input?.value);
  if (!parsed) {
    if (status) {
      status.textContent = 'Enter latitude / longitude in DMS or decimal format.';
  ...
  if (status) {
    status.textContent = `${parsed.lat.toFixed(6)}, ${parsed.lng.toFixed(6)}`;
...
function bindUI() {
  ...
  document.getElementById('toggleNewDeals')?.addEventListener('change', e => {
    ensureNewDealsLayerVisible(e.target.checked);
    if (!e.target.checked && state.newDealsAddMode) setNewDealAddMode(false);
  });
