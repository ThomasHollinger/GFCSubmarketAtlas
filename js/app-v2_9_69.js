...
function updateNewDealsAuthUI(message = '') {
  ...
  const syncBtn = document.getElementById('syncLocalNewDealsBtn');
  const configured = firebaseNewDealsConfigured();
  const authorized = newDealAuthorizedUser(state.newDealsUser);
  const localCount = readLocalNewDeals().length;
  if (syncBtn) syncBtn.hidden = !(configured && localCount > 0);
  if (status) {
    ...
    else if (state.newDealsCloudLoaded) status.textContent = 'Shared live • enter team password only when adding or deleting a pin';
...
function updateNewDealsUI() {
  ...
  if (hint) hint.textContent = state.newDealsAddMode ? 'Click anywhere on the map to place the new deal.' : 'Everyone can view shared New Deals. The team password is required only to add or delete pins.';
...
async function syncLocalNewDealsToCloud() {
  if (!await authorizeNewDealEditing()) return;
  const localDeals = readLocalNewDeals();
  if (!localDeals.length) { alert('There are no browser-saved New Deals to sync.'); return; }
  if (!confirm(`Sync ${localDeals.length} browser-saved New Deals into the shared database? Existing matching IDs will be updated.`)) return;
  const db = firebase.firestore();
  const batch = db.batch();
  localDeals.forEach(deal => {
    const id = deal.id || `deal-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;
    batch.set(db.collection(NEW_DEALS_COLLECTION).doc(id), {
      name: String(deal.name || 'New Deal'), lat: Number(deal.lat), lng: Number(deal.lng),
      city: String(deal.city || ''), submarket: String(deal.submarket || ''),
      createdAt: deal.createdAt || new Date().toISOString(), updatedAt: new Date().toISOString()
    }, { merge: true });
  });
  try {
    await batch.commit();
    updateNewDealsAuthUI(`${localDeals.length} existing browser pins synced to the shared database.`);
  } catch (err) {
    console.error(err); alert(`Existing pins could not be synced: ${err.message || err}`);
  }
}
...
function rebuildNewDealsLayer() {
  ...
  state.newDealsLayer = L.layerGroup((state.newDeals || []).map(deal => {
    const marker = L.marker([Number(deal.lat), Number(deal.lng)], { icon: newDealIcon(deal), zIndexOffset: 1200 });
...
function bindUI() {
  ...
  document.getElementById('syncLocalNewDealsBtn')?.addEventListener('click', syncLocalNewDealsToCloud);
