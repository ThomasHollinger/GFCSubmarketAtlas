...
function bindAddressCopyPopup(marker, record) {
  marker.bindPopup(`
    <div class="address-copy-popup">
      ...
      <button type="button" class="address-copy-btn">Copy Address</button>
  ...
  marker.on('popupopen', event => {
    ...
    const button = popupElement?.querySelector('.address-copy-btn');
    ...
    button.onclick = async clickEvent => {
      ...
      try {
        await copyAddressToClipboard(record.fullAddress);
        button.textContent = 'Copied!';
        if (status) status.textContent = 'Address copied to clipboard.';
      } catch (err) {
        console.warn('Could not copy address to clipboard', err);
        button.textContent = 'Copy failed';
        if (status) status.textContent = 'Select the address above and copy it manually.';
      }
    };
  });
}
