  ...
  return `
    <div class="radius-prompt-card ${mode === 'quickview' ? 'radius-prompt-card-quickview' : 'radius-prompt-card-snapshot'}">
      ...
      <div class="radius-prompt-actions">
        <button type="button" class="${radiusPromptButtonClass(mode)} radius-prompt-yes">${escapeHtml(yesLabel)}</button>
        <button type="button" class="radius-prompt-no radius-prompt-cancel">${escapeHtml(noLabel)}</button>
      </div>
    </div>`;
}
...
function showRadiusPrompt(mode, centerLatLng, radiusMiles) {
  ...
  marker.on('add', () => {
    ...
    if (!root) return;
    const yes = root.querySelector('.radius-prompt-yes');
    const no = root.querySelector('.radius-prompt-cancel');
