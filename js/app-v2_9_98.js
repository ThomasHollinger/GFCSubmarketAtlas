...
function showRadiusPrompt(mode, centerLatLng, radiusMiles) {
  ...
  // This marker sits directly on the Leaflet map. Prevent button clicks from
  // bubbling into the map's click handler, which would otherwise reopen the
  // radius prompt immediately and swallow the Preview action.
  const markerEl = marker.getElement?.();
  if (markerEl) {
    L.DomEvent.disableClickPropagation(markerEl);
    L.DomEvent.on(markerEl, 'click', (evt) => {
      if (evt) L.DomEvent.stop(evt);
    });
    markerEl.querySelector('[data-radius-prompt-action="yes"]')?.addEventListener('click', (evt) => {
      evt.preventDefault();
      evt.stopPropagation();
      evt.stopImmediatePropagation?.();
      openRadiusPromptReport(mode);
    });
    markerEl.querySelector('[data-radius-prompt-action="no"]')?.addEventListener('click', (evt) => {
      evt.preventDefault();
      evt.stopPropagation();
      evt.stopImmediatePropagation?.();
...
function newDealPopupContent(deal) {
  ...
  // Keep all popup button clicks inside the popup. Leaflet's map-level click
  // handler must not see these clicks as map clicks.
  ...
  wrap.querySelectorAll('.new-deal-preview-btn').forEach(btn => {
    btn.addEventListener('click', (event) => {
      ...
      if (event && event.nativeEvent) L.DomEvent.stop(event.nativeEvent);
