...
function newDealPlaceIsAddress(feature) {
  if (feature?.provider === 'arcgis') return true;
  const p = feature?.properties || {};
...
function showNewDealPlaceResultMarker(lat, lng, label) {
  ...
  const icon = L.divIcon({
    className: '',
    html: '<div class="new-deal-marker">S</div>',
    iconSize: [24, 24],
    iconAnchor: [12, 12],
    popupAnchor: [0, -12]
  });
...
async function fetchArcGisAddressSuggestions(q, signal) {
  const ctx = newDealSearchLocationParams({});
  const params = new URLSearchParams({ f: 'json', text: q, maxSuggestions: '8', countryCode: 'USA' });
  if (ctx.center) params.set('location', `${ctx.center.lng},${ctx.center.lat}`);
  const response = await fetch(`${NEW_DEAL_ADDRESS_SUGGEST_URL}?${params.toString()}`, {
    signal,
    headers: { Accept: 'application/json' }
  });
  if (!response.ok) throw new Error(`Address service returned ${response.status}`);
  const data = await response.json();
  if (data?.error) throw new Error(data.error.message || 'Address suggestion failed');
...
async function fetchNewDealPlaceSuggestions() {
  ...
  try {
    ...
    if (isNewDealAddressQuery(q)) {
      ...
      suggestions = await fetchArcGisAddressSuggestions(q, newDealPlaceSearchAbort.signal);
      // Fall back to Photon only if ArcGIS returns nothing.
...
async function selectNewDealPlaceSuggestion(index) {
  ...
  try {
    ...
    state.map?.flyTo([lat, lng], feature.provider === 'arcgis' ? 18 : (newDealPlaceIsAddress(feature) ? 18 : 17), { animate: true, duration: 0.9 });
