...
function renderRadiusPromptHtml(mode) {
  ...
  return `
    <div class="radius-prompt-card ${mode === 'quickview' ? 'radius-prompt-card-quickview' : 'radius-prompt-card-snapshot'}" data-radius-prompt-mode="${escapeHtml(mode)}">
      <div class="radius-prompt-title">${escapeHtml(title)}</div>
      <div class="radius-prompt-actions">
        <button type="button" class="${radiusPromptButtonClass(mode)}" data-radius-prompt-action="yes">${escapeHtml(yesLabel)}</button>
        <button type="button" class="radius-prompt-no" data-radius-prompt-action="no">${escapeHtml(noLabel)}</button>
      </div>
    </div>`;
}
...
function showRadiusPrompt(mode, centerLatLng, radiusMiles) {
  ...
  const promptCard = markerEl?.querySelector?.('.radius-prompt-card');
  if (promptCard) {
    L.DomEvent.disableClickPropagation(promptCard);
    const yesBtn = promptCard.querySelector('[data-radius-prompt-action="yes"]');
    const noBtn = promptCard.querySelector('[data-radius-prompt-action="no"]');
...
window.GCSA_OPEN_MARKET_PREVIEW = () => {
...
function buildMarketQuickviewHtml(data) {
  ...
  const incomeBandLowerBound = label => {
...
function cleanupNewDealMove(ctx, restoreOriginal = false) {
  if (!ctx) return;
  try {
    if (ctx.dragEndHandler) ctx.marker.off('dragend', ctx.dragEndHandler);
  } catch (_) {}
  try { if (ctx.marker.dragging) ctx.marker.dragging.disable(); } catch (_) {}
  try { if (restoreOriginal && ctx.original) ctx.marker.setLatLng(ctx.original); } catch (_) {}
  try { ctx.marker.getElement()?.classList.remove('new-deal-move-active'); } catch (_) {}
  try { ctx.marker.closeTooltip(); ctx.marker.unbindTooltip(); } catch (_) {}
  if (activeNewDealMove === ctx) activeNewDealMove = null;
}

function cancelActiveNewDealMove() {
  if (!activeNewDealMove) return false;
  const ctx = activeNewDealMove;
  cleanupNewDealMove(ctx, true);
  state.map?.closePopup();
  updateNewDealsUI();
  return true;
}

if (!window.__gcsaNewDealEscapeHandlerInstalled) {
  window.__gcsaNewDealEscapeHandlerInstalled = true;
  document.addEventListener('keydown', (event) => {
    if (event.key === 'Escape' && activeNewDealMove) {
      event.preventDefault();
      event.stopPropagation();
      cancelActiveNewDealMove();
    }
  }, true);
}
...
async function moveNewDeal(deal) {
  ...
  if (activeNewDealMove) cancelActiveNewDealMove();
  ...
  marker.bindTooltip('Drag this pin to its new location, then release. Press Esc to cancel.', {
    direction: 'top', offset: [0, -14], className: 'new-deal-move-tooltip'
  }).openTooltip();

  const ctx = { marker, original, dragEndHandler: null };
  const dragEndHandler = async () => {
    if (activeNewDealMove !== ctx) return;
    cleanupNewDealMove(ctx, false);
  ...
  ctx.dragEndHandler = dragEndHandler;
  activeNewDealMove = ctx;
  marker.once('dragend', dragEndHandler);
}
...
function newDealPopupContent(deal) {
  ...
  wrap.innerHTML = `<h3>${escapeHtml(deal.name || 'New Deal')}</h3>
    ...
    <div class="new-deal-popup-actions">
      <button type="button" class="new-deal-preview-btn" data-radius="3" data-deal-id="${escapeHtml(deal.id || '')}">3 Mile Preview</button>
      <button type="button" class="new-deal-preview-btn" data-radius="5" data-deal-id="${escapeHtml(deal.id || '')}">5 Mile Preview</button>
      <button type="button" class="new-deal-move-btn" data-deal-id="${escapeHtml(deal.id || '')}">Move Pin</button>
      <button type="button" class="new-deal-delete-btn" data-deal-id="${escapeHtml(deal.id || '')}">Delete Pin</button>
    </div>`;
...
function bindUI() {
  ...
  if (snapshotButton) {
    snapshotButton.addEventListener('click', ev => {
      ev.preventDefault();
      ev.stopPropagation();
      window.GCSA_OPEN_MARKET_PREVIEW();
    });
    snapshotButton.setAttribute('onclick', 'window.GCSA_OPEN_MARKET_PREVIEW()');
  }
  ...
  document.addEventListener('click', ev => {
    const promptBtn = ev.target?.closest?.('[data-radius-prompt-action]');
    if (promptBtn) {
      const card = promptBtn.closest('.radius-prompt-card');
      const mode = card?.dataset.radiusPromptMode;
      if (mode) {
        ev.preventDefault();
        ev.stopPropagation();
        ev.stopImmediatePropagation?.();
        if (promptBtn.dataset.radiusPromptAction === 'yes') openRadiusPromptReport(mode);
        else cancelRadiusPrompt(mode);
      }
    }
  }, true);
  updateMarketSnapshotUI();
