...
function renderSelected(p) {
  ...
  const demo = demoForSubmarketID(p.SubmarketID);
  document.getElementById('selectedPanel').classList.remove('empty');
  document.getElementById('selectedPanel').innerHTML = `
...
function bindUI() {
  ...
  document.getElementById('mapThemeSelect').addEventListener('change', async e => {
    if ((e.target.value === 'income' || e.target.value === 'growth') && !state.demographicsLoaded) {
      document.getElementById('toggleDemographics').checked = true;
      await loadDemographics();
    }
