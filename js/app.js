...
function poiCategory(tags) {
  ...
  if (['supermarket','grocery'].includes(shop)) return 'Grocery';
...
function overpassQuery() {
  ...
  const shop = 'supermarket|grocery|convenience|department_store|mall|clothes|shoes|hardware|doityourself|furniture|electronics|sports|variety_store|discount|general|beauty|bakery|chemist|pharmacy|car|car_parts|mobile_phone|jewelry|florist|gift|optician|pet|toys|books';
...
function styleFeature(feature) {
  ...
  return {
    ...
    fillColor: state.mapTheme === 'schools' ? colorForSchoolScore(scoreSummaryForSubmarket(p.DisplayName).overall) : (state.mapTheme === 'retail' ? colorForRetailDensity(retailSummaryForSubmarket(p.SubmarketID, p.AreaSqMi).density) : (p.HubColor || p.HubBaseColor || '#8ea0ad')),
...
async function loadSchools() {
  ...
  state.schoolLayer = L.geoJSON({ type: 'FeatureCollection', features: state.schools }, {
    ...
    onEachFeature: (feature, layer) => {
      const p = feature.properties;
      layer.bindPopup(`<div class="school-popup"><h3>${p.NAME}</h3><p><b>Type:</b> ${p.SchoolType}</p><p><b>GreatSchools:</b> ${p.GreatSchoolsRating ? p.GreatSchoolsRating + '/10' : (p.SchoolType === 'Other' ? 'Not Rated (Excluded)' : 'Not Rated')}</p><p><b>Location:</b> ${p.CITY}, ${p.STATE}</p><p><b>County:</b> ${p.NMCNTY || ''}</p><p><b>Submarket:</b> ${p.SubmarketName || 'Outside submarket boundary'}</p><p><b>NCES ID:</b> ${p.NCESSCH || ''}</p></div>`);
