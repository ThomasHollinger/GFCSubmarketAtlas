...
function renderNewDealPlaceSuggestions(features) {
  const box = document.getElementById('newDealPlaceSuggestions');
  if (!box) return;
  newDealPlaceSuggestions = Array.isArray(features) ? features : [];
  newDealPlaceActiveIndex = -1;
  box.innerHTML = '';
...
async function fetchNewDealPlaceSuggestions() {
  ...
  try {
    ...
    renderNewDealPlaceSuggestions(features);
    if (status) status.textContent = features.length ? `${features.length} suggestion${features.length === 1 ? '' : 's'}` : 'No matching places found.';
  } catch (err) {
...
function selectNewDealPlaceSuggestion(index) {
  ...
  const targetZoom = props.type === 'house' || props.osm_value === 'house' ? 18 : 17;
