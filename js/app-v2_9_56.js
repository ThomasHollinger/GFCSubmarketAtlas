const state = {
  ...
  snapshotAcs: { attributes: null, attributesLoaded: false, attributesPromise: null, radiusCache: new Map() }
};
...
const SNAPSHOT_ACS_CACHE_KEY = 'gcsa-market-snapshot-acs2024-v1';
const SNAPSHOT_ACS_CACHE_TTL_MS = 30 * 24 * 60 * 60 * 1000;
const SNAPSHOT_ACS_COUNTIES = [
  ['01', '003'], // Baldwin County, AL
  ['01', '097'], // Mobile County, AL
...
const TIGERWEB_ACS_BG_QUERY_URL = 'https://tigerweb.geo.census.gov/arcgis/rest/services/TIGERweb/Tracts_Blocks/MapServer/8/query';
const CENSUS_ACS5_URL = 'https://api.census.gov/data/2024/acs/acs5';
...
function setMarketSnapshotMode(active, radiusMiles = null) {
  state.marketSnapshot = state.marketSnapshot || { active: false, radiusMiles: null, awaitingPoint: false, busy: false, pendingCenter: null, radiusLayer: null, radiusCircle: null, promptMarker: null };
  const prevRadius = state.marketSnapshot.radiusMiles;
  ...
  } else if (prevRadius !== null && prevRadius !== undefined && state.marketSnapshot.radiusMiles !== prevRadius) {
    clearRadiusPromptOverlay('snapshot');
  }
...
function quickviewSignedValueHtml(value, kind = 'int') {
  ...
  const intensity = kind === 'pct'
    ? Math.max(32, 58 - (abs / 150) * 22)
    : Math.max(30, 56 - (abs / 500) * 20);
  const color = num > 0
    ? `hsl(142 68% ${intensity}%)`
    : `hsl(0 72% ${intensity}%)`;
...
function radiusModeState(mode) {
  return mode === 'quickview' ? state.marketQuickview : state.marketSnapshot;
}

function radiusPromptLabel(mode) {
  return mode === 'quickview' ? 'See Market Quickview?' : 'See Market Snapshot?';
}

function radiusPromptButtonClass(mode) {
  return mode === 'quickview' ? 'radius-prompt-yes quickview' : 'radius-prompt-yes snapshot';
}
...
function renderRadiusPromptHtml(mode) {
  const yesLabel = mode === 'quickview' ? 'Open Quickview' : 'Open Snapshot';
  const noLabel = 'No';
  const title = radiusPromptLabel(mode);
  return `
    <div class="radius-prompt-card ${mode === 'quickview' ? 'radius-prompt-card-quickview' : 'radius-prompt-card-snapshot'}" onclick="event.stopPropagation()">
      <div class="radius-prompt-title">${escapeHtml(title)}</div>
      <div class="radius-prompt-actions">
        <button type="button" class="${radiusPromptButtonClass(mode)}" onclick="event.stopPropagation(); window.GCSA_RADIUS_PROMPT_YES('${mode}')">${escapeHtml(yesLabel)}</button>
        <button type="button" class="radius-prompt-no" onclick="event.stopPropagation(); window.GCSA_RADIUS_PROMPT_NO('${mode}')">${escapeHtml(noLabel)}</button>
      </div>
    </div>`;
}
...
function showRadiusPrompt(mode, centerLatLng, radiusMiles) {
  ...
  const circle = L.circle([s.pendingCenter.lat, s.pendingCenter.lng], {
  ...
  s.radiusCircle = circle;
  const bounds = circle.getBounds && circle.getBounds();
  if (bounds && typeof bounds.isValid === 'function' ? bounds.isValid() : true) {
    state.map.fitBounds(bounds, { padding: [60, 60], animate: true });
  }
  const marker = L.marker([s.pendingCenter.lat, s.pendingCenter.lng], {
    interactive: true,
    keyboard: false,
    zIndexOffset: 2200,
    icon: L.divIcon({
      className: '',
      html: renderRadiusPromptHtml(mode),
      iconSize: [210, 86],
      iconAnchor: [105, 43]
    })
  }).addTo(layer);
  s.promptMarker = marker;
}

function openRadiusPromptReport(mode) {
  if (mode === 'quickview') {
    openMarketQuickviewReportFromPrompt();
  } else {
    openMarketSnapshotReportFromPrompt();
  }
}
...
window.GCSA_RADIUS_PROMPT_YES = openRadiusPromptReport;
window.GCSA_RADIUS_PROMPT_NO = cancelRadiusPrompt;
...
function buildMarketQuickviewHtml(data) {
  ...
  const legacyIncomeBandOrder = ['Less than 25k', '25k - 50k', '50k - 75k', '75k - 100k', '100k - 125k', '125k - 150k', '150k - 200k', 'Above 200k'];
  const legacyIncomeBandRank = label => {
    const normalized = String(label || '').toLowerCase().replace(/\$/g, '').replace(/,/g, '').replace(/\s+/g, ' ').trim();
    const index = legacyIncomeBandOrder.findIndex(band => String(band).toLowerCase().replace(/\$/g, '').replace(/,/g, '').replace(/\s+/g, ' ').trim() === normalized);
    return index >= 0 ? index : legacyIncomeBandOrder.length;
  };
...
function quickviewAggregateBlocks(features) {
  ...
  const normalizeIncomeBandLabel = label => String(label || '')
    ...
    .replace(/\s+/g, ' ')
    .trim();
  const incomeBandRank = label => {
    const normalized = normalizeIncomeBandLabel(label);
    const rankMap = new Map(incomeBandOrder.map((band, index) => [normalizeIncomeBandLabel(band), index]));
    return rankMap.has(normalized) ? rankMap.get(normalized) : incomeBandOrder.length;
  };
...
function snapshotAcsNumber(value) {
  const n = Number(value);
  return Number.isFinite(n) && n >= 0 ? n : null;
}

function snapshotAcsStoredAttributes() {
  try {
    const raw = localStorage.getItem(SNAPSHOT_ACS_CACHE_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    if (!parsed || !parsed.savedAt || !parsed.records || (Date.now() - parsed.savedAt) > SNAPSHOT_ACS_CACHE_TTL_MS) return null;
...
function snapshotAcsPersistAttributes(records) {
  try {
    localStorage.setItem(SNAPSHOT_ACS_CACHE_KEY, JSON.stringify({ savedAt: Date.now(), records }));
  } catch (err) {
    // localStorage is an optimization only; Snapshot still works without it.
  }
}

async function ensureSnapshotAcsAttributes() {
  state.snapshotAcs = state.snapshotAcs || { attributes: null, attributesLoaded: false, attributesPromise: null, radiusCache: new Map() };
  if (state.snapshotAcs.attributesLoaded && state.snapshotAcs.attributes) return state.snapshotAcs.attributes;
  if (state.snapshotAcs.attributesPromise) return state.snapshotAcs.attributesPromise;

  const stored = snapshotAcsStoredAttributes();
  if (stored && typeof stored === 'object') {
    state.snapshotAcs.attributes = stored;
    state.snapshotAcs.attributesLoaded = true;
    return stored;
  }

  state.snapshotAcs.attributesPromise = (async () => {
    const countyResults = await Promise.allSettled(SNAPSHOT_ACS_COUNTIES.map(async ([stateFips, countyFips]) => {
      const params = new URLSearchParams({
        get: 'NAME,B01003_001E,B11001_001E,B19013_001E,B01002_001E',
        for: 'block group:*',
        in: `state:${stateFips} county:${countyFips} tract:*`
      });
      const response = await fetch(`${CENSUS_ACS5_URL}?${params.toString()}`);
      if (!response.ok) throw new Error(`ACS request failed (${response.status}) for ${stateFips}-${countyFips}`);
      return response.json();
    }));
    const countyRows = countyResults.filter(result => result.status === 'fulfilled').map(result => result.value);
    if (!countyRows.length) throw new Error('No ACS county block-group data could be loaded');

    const records = {};
    countyRows.forEach(rows => {
      if (!Array.isArray(rows) || rows.length < 2) return;
      const header = rows[0];
      const index = Object.fromEntries(header.map((name, i) => [name, i]));
      rows.slice(1).forEach(row => {
        const stateFips = String(row[index.state] || '').padStart(2, '0');
        const countyFips = String(row[index.county] || '').padStart(3, '0');
        const tract = String(row[index.tract] || '').padStart(6, '0');
        const blockGroup = String(row[index['block group']] || '');
        const geoid = `${stateFips}${countyFips}${tract}${blockGroup}`;
        if (geoid.length !== 12) return;
        records[geoid] = {
          population: snapshotAcsNumber(row[index.B01003_001E]),
          households: snapshotAcsNumber(row[index.B11001_001E]),
          median_household_income: snapshotAcsNumber(row[index.B19013_001E]),
          median_age: snapshotAcsNumber(row[index.B01002_001E])
        };
      });
    });
    state.snapshotAcs.attributes = records;
    state.snapshotAcs.attributesLoaded = true;
    if (countyRows.length === SNAPSHOT_ACS_COUNTIES.length) snapshotAcsPersistAttributes(records);
    return records;
  })();

  try {
    return await state.snapshotAcs.attributesPromise;
  } finally {
    state.snapshotAcs.attributesPromise = null;
  }
}

function snapshotCircleOverlapArea(radiusA, radiusB, centerDistance) {
  const r1 = Math.max(0, Number(radiusA) || 0);
  const r2 = Math.max(0, Number(radiusB) || 0);
  const d = Math.max(0, Number(centerDistance) || 0);
  if (!r1 || !r2) return 0;
  if (d >= r1 + r2) return 0;
  if (d <= Math.abs(r1 - r2)) return Math.PI * Math.min(r1, r2) ** 2;
  const clamp = value => Math.max(-1, Math.min(1, value));
  const a1 = r1 * r1 * Math.acos(clamp((d * d + r1 * r1 - r2 * r2) / (2 * d * r1)));
  const a2 = r2 * r2 * Math.acos(clamp((d * d + r2 * r2 - r1 * r1) / (2 * d * r2)));
  const radical = Math.max(0, (-d + r1 + r2) * (d + r1 - r2) * (d - r1 + r2) * (d + r1 + r2));
  return a1 + a2 - 0.5 * Math.sqrt(radical);
}

function snapshotBlockGroupWeight(centerLatLng, radiusMiles, tigerAttributes) {
  const lat = Number(tigerAttributes.CENTLAT);
  const lon = Number(tigerAttributes.CENTLON);
  const landSqMeters = Number(tigerAttributes.AREALAND);
  if (!Number.isFinite(lat) || !Number.isFinite(lon) || !Number.isFinite(landSqMeters) || landSqMeters <= 0) return 1;
  const distanceMiles = centerLatLng.distanceTo(L.latLng(lat, lon)) / 1609.344;
  const landSqMiles = landSqMeters / 2589988.110336;
  const bgRadiusMiles = Math.sqrt(Math.max(landSqMiles, 0) / Math.PI);
  if (!bgRadiusMiles) return distanceMiles <= radiusMiles ? 1 : 0;
  const overlap = snapshotCircleOverlapArea(radiusMiles, bgRadiusMiles, distanceMiles);
  const bgArea = Math.PI * bgRadiusMiles * bgRadiusMiles;
  return Math.max(0, Math.min(1, bgArea ? overlap / bgArea : 0));
}

async function querySnapshotAcsBlockGroups(centerLatLng, radiusMiles) {
  const key = `${centerLatLng.lat.toFixed(5)},${centerLatLng.lng.toFixed(5)}:${Number(radiusMiles)}`;
  state.snapshotAcs = state.snapshotAcs || { attributes: null, attributesLoaded: false, attributesPromise: null, radiusCache: new Map() };
  state.snapshotAcs.radiusCache = state.snapshotAcs.radiusCache || new Map();
  if (state.snapshotAcs.radiusCache.has(key)) return state.snapshotAcs.radiusCache.get(key);

  const params = new URLSearchParams({
    where: '1=1',
    geometry: `${centerLatLng.lng},${centerLatLng.lat}`,
    geometryType: 'esriGeometryPoint',
    inSR: '4326',
    spatialRel: 'esriSpatialRelIntersects',
    distance: String(radiusMiles),
    units: 'esriSRUnit_StatuteMile',
    outFields: 'GEOID,STATE,COUNTY,TRACT,BLKGRP,AREALAND,CENTLAT,CENTLON',
    returnGeometry: 'false',
    f: 'json'
  });
  const response = await fetch(`${TIGERWEB_ACS_BG_QUERY_URL}?${params.toString()}`);
  if (!response.ok) throw new Error(`TIGERweb request failed (${response.status})`);
  const payload = await response.json();
  if (payload.error) throw new Error(payload.error.message || 'TIGERweb radius query failed');
  const rows = (payload.features || []).map(feature => feature.attributes || {}).filter(attrs => attrs.GEOID);
  state.snapshotAcs.radiusCache.set(key, rows);
  return rows;
}

async function loadMarketSnapshotAcs(centerLatLng, radiusMiles) {
  const [attributes, tigerRows] = await Promise.all([
    ensureSnapshotAcsAttributes(),
    querySnapshotAcsBlockGroups(centerLatLng, radiusMiles)
  ]);
  ...
  let ageWeight = 0;
  let contributing = 0;
  let rawIntersecting = 0;

  tigerRows.forEach(row => {
    const values = attributes[String(row.GEOID)];
    if (!values) return;
    rawIntersecting += 1;
    const weight = snapshotBlockGroupWeight(centerLatLng, radiusMiles, row);
    if (!(weight > 0)) return;
    contributing += 1;
    const pop = Number(values.population);
    const hh = Number(values.households);
    if (Number.isFinite(pop) && pop >= 0) population += pop * weight;
    if (Number.isFinite(hh) && hh >= 0) households += hh * weight;
    if (Number.isFinite(values.median_household_income) && values.median_household_income >= 0 && Number.isFinite(hh) && hh > 0) {
      incomeWeighted += values.median_household_income * hh * weight;
      incomeWeight += hh * weight;
    }
    if (Number.isFinite(values.median_age) && values.median_age >= 0 && Number.isFinite(pop) && pop > 0) {
      ageWeighted += values.median_age * pop * weight;
      ageWeight += pop * weight;
    }
  });

  return {
    loaded: true,
    blockGroupCount: contributing,
    intersectingBlockGroupCount: rawIntersecting,
    population: Math.round(population),
    households: Math.round(households),
    median_household_income: incomeWeight > 0 ? incomeWeighted / incomeWeight : null,
    median_age: ageWeight > 0 ? ageWeighted / ageWeight : null
  };
}

function buildMarketSnapshotHtml(centerLatLng, radiusMiles, snapshotAcs = null) {
  ...
  const demographics = snapshotAcs && snapshotAcs.loaded ? snapshotAcs : null;
  ...
  const demographicsLoaded = !!(snapshotAcs && snapshotAcs.loaded);
  ...
  const demographicsSectionHtml = snapshotAcs && snapshotAcs.error
    ? `<div class="snapshot-empty">ACS radius demographics could not be loaded: ${escapeHtml(snapshotAcs.error)}</div>`
    : (!demographicsLoaded
      ? '<div class="snapshot-empty">ACS radius demographics are loading.</div>'
      : (demographics && demographics.blockGroupCount > 0
        ? `<div class="snapshot-metric-grid">
            ${renderSnapshotMetric('Population', fmt(demographics.population))}
            ${renderSnapshotMetric('Households', fmt(demographics.households))}
            ${renderSnapshotMetric('Median Income', fmtMoney(demographics.median_household_income))}
            ${renderSnapshotMetric('Median Age', fmtOne(demographics.median_age))}
          </div>
          <div class="snapshot-subnote">ACS 2020-2024 5-Year block groups intersecting the selected radius. Partial block groups are apportioned using an area-overlap estimate so Snapshot is driven by the clicked radius, not by submarket centroids.</div>`
        : '<div class="snapshot-empty">No ACS block-group demographic overlap was found for this radius.</div>'));
  ...
  return `
    ...
    <section class="snapshot-section">
      <div class="snapshot-section-head"><h4>Demographics</h4><span>${demographicsLoaded && demographics ? demographics.blockGroupCount.toLocaleString() + ' ACS block groups' : (snapshotAcs && snapshotAcs.error ? 'Unavailable' : 'Still loading')}</span></div>
...
async function openMarketSnapshotReportFromPrompt() {
  const s = state.marketSnapshot || {};
  if (!s.pendingCenter || !s.radiusMiles) return;
  const center = s.pendingCenter instanceof L.LatLng ? s.pendingCenter : L.latLng(s.pendingCenter.lat, s.pendingCenter.lng);
  const subtitle = `${marketSnapshotRadiusLabel(s.radiusMiles)} centered at ${center.lat.toFixed(5)}, ${center.lng.toFixed(5)}`;
  openMarketSnapshotModal('Market Snapshot', subtitle, '<div class="snapshot-empty">Loading radius data and ACS block-group demographics...</div>');

  const layerPromise = ensureSnapshotDataLoaded();
  let snapshotAcs;
  try {
    snapshotAcs = await loadMarketSnapshotAcs(center, s.radiusMiles);
  } catch (err) {
    console.warn('ACS radius demographics unavailable for Market Snapshot', err);
    snapshotAcs = { loaded: false, error: err && err.message ? err.message : 'Unknown ACS radius error' };
  }
  const html = buildMarketSnapshotHtml(center, s.radiusMiles, snapshotAcs);
  openMarketSnapshotModal('Market Snapshot', subtitle, html);
  void layerPromise.then(() => {
    const latest = state.marketSnapshot || {};
    const sameCenter = latest.pendingCenter && Math.abs(Number(latest.pendingCenter.lat) - center.lat) < 1e-7 && Math.abs(Number(latest.pendingCenter.lng) - center.lng) < 1e-7;
    if (sameCenter && Number(latest.radiusMiles) === Number(s.radiusMiles)) {
      openMarketSnapshotModal('Market Snapshot', subtitle, buildMarketSnapshotHtml(center, s.radiusMiles, snapshotAcs));
    }
  });
}
...
loadData()
  .then(() => {
    // Warm ACS block-group attributes for Market Snapshot. This is separate from Market Quickview.
    setTimeout(() => {
      ensureSnapshotAcsAttributes().catch(err => console.warn('Background Market Snapshot ACS preload failed', err));
    }, 250);
