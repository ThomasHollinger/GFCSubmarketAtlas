...
function updateNewDealsUI() {
  ...
  if (hint) hint.textContent = state.newDealsAddMode ? 'Click anywhere on the map to place the new deal.' : (newDealAuthorizedUser(state.newDealsUser) ? 'Private Deals & Leads unlocked. Add, move, convert, and delete are enabled.' : 'Deals and leads are private. Enter the team password to view the pins.');
...
async function openNewDealEditor(latlng) {
  ...
  const save=async()=>{
    ...
    const deal={name,lat:latlng.lat,lng:latlng.lng,city:ctx.city,submarket:ctx.submarket,category:NEW_DEAL_CATEGORY.NEW_DEAL,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};
    try {
...
function bindUI() {
  ...
  const handlePrivateDealCategoryToggle = async (e, category) => {
    ...
    if (category === NEW_DEAL_CATEGORY.NEW_DEAL && !e.target.checked && state.newDealsAddMode) await setNewDealAddMode(false);
    rebuildNewDealsLayer();
