...
async function authorizeNewDealEditing() {
  ...
  if (!password) {
    alert('A password is required to edit New Deals.');
    return false;
  }
  try {
    await firebase.auth().setPersistence(firebase.auth.Auth.Persistence.SESSION);
    const result = await firebase.auth().signInWithEmailAndPassword(newDealEditorEmail(), password);
    if (!newDealAuthorizedUser(result.user)) {
      await firebase.auth().signOut();
      alert('This account is not authorized to edit New Deals.');
      return false;
    }
    return true;
  } catch (err) {
...
function updateNewDealsAuthUI(message = '') {
  ...
  if (status) {
    ...
    else if (authorized) status.textContent = 'Shared live • editing unlocked for this session';
    else if (state.newDealsCloudLoaded) status.textContent = 'Shared live • enter team password only when editing a pin';
    else status.textContent = 'Connecting to shared New Deals…';
  }
  const addBtn = document.getElementById('addNewDealPin');
  if (addBtn) addBtn.disabled = !configured;
}
...
function updateNewDealsUI() {
  ...
  if (hint) hint.textContent = state.newDealsAddMode ? 'Click anywhere on the map to place the new deal.' : 'Everyone can view shared New Deals. The team password is required only to add, move, or delete pins.';
...
async function initializeFirebaseNewDeals() {
  ...
  try {
    ...
    if (state.newDealsUnsubscribe) { state.newDealsUnsubscribe(); state.newDealsUnsubscribe = null; }
    const db = firebase.firestore();
    state.newDealsUnsubscribe = db.collection(NEW_DEALS_COLLECTION).onSnapshot(snapshot => {
      state.newDeals = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })).filter(d => Number.isFinite(Number(d.lat)) && Number.isFinite(Number(d.lng)));
      ...
      console.error('New Deals Firestore listener failed', err);
      updateNewDealsAuthUI('Shared New Deals could not be loaded. Check Firebase permissions.');
    });
    updateNewDealsAuthUI();
    return true;
  } catch (err) {
...
function newDealPopupContent(deal) {
  ...
  if (move) move.addEventListener('click', () => moveNewDeal(deal));
...
function rebuildNewDealsLayer() {
  ...
  state.newDealsLayer = L.layerGroup((state.newDeals || []).map(deal => {
    const marker = L.marker([Number(deal.lat), Number(deal.lng)], { icon: newDealIcon(deal), zIndexOffset: 1200, newDealId: deal.id });
    marker.bindPopup(() => newDealPopupContent(deal), { minWidth: 220 }); return marker;
  }));
...
function rankNewDealPlaceSuggestions(features, q) {
  const rows = Array.isArray(features) ? features.slice() : [];
  if (!isNewDealAddressQuery(q)) return rows;
  return rows
    .map((feature, index) => ({ feature, index, address: newDealPlaceIsAddress(feature) ? 1 : 0 }))
    .sort((a, b) => (b.address - a.address) || (a.index - b.index))
    .map(row => row.feature);
}
...
function showNewDealPlaceResultMarker(lat, lng, label) {
  ...
  if (label) newDealPlaceResultMarker.bindTooltip(label, { direction: 'top', offset: [0, -8] });
}

function renderNewDealPlaceSuggestions(features, q = '') {
  const box = document.getElementById('newDealPlaceSuggestions');
  if (!box) return;
  newDealPlaceSuggestions = rankNewDealPlaceSuggestions(features, q);
  newDealPlaceActiveIndex = -1;
  box.innerHTML = '';
...
async function fetchNewDealPlaceSuggestions() {
  ...
  if (q.length < 3) { clearNewDealPlaceSuggestions(); if (status) status.textContent = q ? 'Type at least 3 characters.' : ''; return; }
  ...
  if (center && Number.isFinite(center.lat) && Number.isFinite(center.lng)) {
    params.set('lat', String(center.lat)); params.set('lon', String(center.lng));
    params.set('zoom', String(Math.max(7, Math.min(16, state.map.getZoom?.() || 9))));
    params.set('location_bias_scale', '0.25');
  }
  if (status) { status.textContent = 'Searching…'; status.classList.remove('error'); }
  try {
    const response = await fetch(`${NEW_DEAL_PLACE_SEARCH_URL}?${params.toString()}`, { signal: newDealPlaceSearchAbort.signal, headers: { Accept: 'application/json' } });
    if (!response.ok) throw new Error(`Search service returned ${response.status}`);
    const data = await response.json();
    const features = (data?.features || []).filter(f => Array.isArray(f?.geometry?.coordinates) && Number.isFinite(Number(f.geometry.coordinates[0])) && Number.isFinite(Number(f.geometry.coordinates[1])));
    renderNewDealPlaceSuggestions(features, q);
    if (status) status.textContent = features.length ? (isNewDealAddressQuery(q) ? `${features.length} address-first suggestion${features.length === 1 ? '' : 's'}` : `${features.length} nearby place suggestion${features.length === 1 ? '' : 's'}`) : 'No matching places found.';
  } catch (err) {
...
function selectNewDealPlaceSuggestion(index) {
  ...
  const status = document.getElementById('newDealPlaceSearchStatus');
  const label = newDealPlaceLabel(feature);
  if (input) input.value = label.detail ? `${label.main}, ${label.detail}` : label.main;
  ...
  const targetZoom = newDealPlaceIsAddress(feature) ? 18 : 17;
  const markerLabel = label.detail ? `${label.main}, ${label.detail}` : label.main;
  showNewDealPlaceResultMarker(lat, lng, markerLabel);
  state.map?.flyTo([lat, lng], targetZoom, { animate: true, duration: 0.9 });
}
...
function bindUI() {
  ...
  newDealPlaceInput?.addEventListener('keydown', e => {
    ...
    else if (e.key === 'Escape') { clearNewDealPlaceSuggestions(); clearNewDealPlaceResultMarker(); }
  });
  newDealPlaceInput?.addEventListener('blur', () => setTimeout(clearNewDealPlaceSuggestions, 120));
  document.addEventListener('click', e => { if (!e.target?.closest?.('.new-deal-place-search')) { clearNewDealPlaceSuggestions(); clearNewDealPlaceResultMarker(); } });
  state.map?.on('click', () => clearNewDealPlaceResultMarker());
